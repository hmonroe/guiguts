...
sub showAllWords {
	my $lang;
	$savedHeader =
	  "Total words: $totalwordcount, Distinct words: $distinctwordcount\n";
...
sub showUnspeltWords {
	if ($unspeltwordcount) {
		$savedHeader =
		  "Spelt words: $speltwordcount, Unspelt words: $unspeltwordcount\n";
	} else {
...
sub showspeltforeignwords {
	...
	if ( $sortorder eq 'f' ) {
		for ( &::natural_sort_freq( \%distinctwords ) ) {
			...
			{
				my $line = sprintf( "%-8d %-6s %s",
									$distinctwords{$_}, $seenwordslang{$_},
									$_ );
				$multiwclistbox->insert( 'end', $line );
				$i++;
			}
...
#	};
	if ($unspeltwordcount) {
		$savedHeader =
		  "Spelt words: $speltwordcount, Spelt foreign words: $i\n";
	} else {
		$savedHeader = "No spelling undertaken!";
	}
	$multiwclistbox->delete('0');
