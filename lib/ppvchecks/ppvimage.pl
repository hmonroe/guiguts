...
my $label1 = $mw -> Label(-text=>"ppvhtml.pl post processing verification image checker.");
...
sub loadFile {
  $srctext = $mw->getOpenFile(-defaultextension => ".htm|.html",
		      -filetypes        =>
		      [['Text Files',       ['.htm', '.html']],
		       ['All Files',        '*',             ],
		      ],
...
#    my($x,$y)=(0,0);
    ...
    if ($keepreading ) {
      ...
      if (/<img/i) { # found one
        $img = $_; $img =~ s/.*<img/<img/i;
        if (not /<img.*>/i) { # tag not closed on same line; get more lines
          $keepreading = 1;
          next;
          }
        else {
    ...
    if ($haveimage) {
      if (not ($img =~ m/\/>/)) {print LOGFILE ("--> $img\n  WARNING: img tag not properly closed\n");}
      $src = $img; $src =~ s/^.*src=['"]([^'" ]*)['"].*$/$1/i;
      print LOGFILE ("\nimage: $src\n");
      if (not ($src =~ m/^images\//)) { # image not in images directory
        print LOGFILE ("  WARNING: image file not in images directory\n");
        }
      my $alt = $img; $alt =~ s/^.*alt=(['"].*$)/$1/i;
      my $altqt = substr($alt,0,1);
      if ($altqt eq '\'') {$alt =~ s/'([^']*)'.*$/$1/; }
      elsif ($altqt eq '"') {$alt =~ s/"([^"]*)".*$/$1/; }
      else {
        print LOGFILE ("  WARNING: <img> has no alt attribute\n");
        $altqt = "";
        }
      if (length($altqt)>0) {print LOGFILE ("  alt=$altqt$alt$altqt\n");}
      my $ttl = $img; $ttl =~ s/^.*title=(['"].*$)/$1/i;
      ...
      if (length($ttlqt)>0) {print LOGFILE ("  title=$ttlqt$ttl$ttlqt\n");}
      my $warn = "  WARNING";
      my $wdstyl = "X";
      my $htstyl = "X";
      # now check for CSS spec
      my $styl = $img; $styl =~ s/^.*style=(.*$)/$1/i;
      if ($styl ne $img) { # we have a style spec
        $wdstyl = $img; $wdstyl =~ s/^.*width *: *([^;]*);.*$/$1/i;
        if ($wdstyl eq $img) { $wdstyl = "X"; }
        $htstyl = $img; $htstyl =~ s/^.*height *: *([^;]*);.*$/$1/i;
      ...
      my $wd = $img; $wd =~ s/^.*width=['"](\d*)['"].*$/$1/i;
      if ($wd eq $img) {
        print LOGFILE ("$warn: <img> lacks width attribute\n");
        $wd = "X";
        }
      if (length($wd)==0) {
        print LOGFILE ("$warn: <img> has empty width attribute\n");
        }
      my $ht = $img; $ht =~ s/^.*height=['"](\d*)['"].*$/$1/i;
      if ($ht eq $img) {
        print LOGFILE ("$warn: <img> lacks height attribute\n");
        $ht = "X";
        }
      if (length($ht)==0) {
...
sub imgdimens {
  ...
  binmode(IMGFILE);
  if ($src =~ /\.png$/) {($x,$y)= pngsize(\*IMGFILE);
    print LOGFILE ("  natural width=\"$x\" height=\"$y\"\n");
    }
  elsif ($src =~ /\.je?pg$/) {($x,$y)= jpegsize(\*IMGFILE);
    print LOGFILE ("  natural width=\"$x\" height=\"$y\"\n");
    }
  elsif ($src =~ /\.gif$/) {($x,$y)= NEWgifsize(\*IMGFILE);
    print LOGFILE ("  natural width=\"$x\" height=\"$y\"\n");
    }
  else {print LOGFILE ("  unknown image type: $src\n"); }
  close IMGFILE;
  my ($dev, $ino, $mode, $nlink, $uid, $gid, $rdev, $size, $atime,
   $mtime, $ctime, $blksize, $blocks) = stat($imgfile);
  ...
  sub bgimgcheck {
    ...
    if ($keepreading ) {
      ...
      if (/background-image/i) { # found one
        $bgimg = $_; $bgimg =~ s/.*background-image/background-image/i;
        if (not /background-image.*;/i) { # spec not closed on same line; get more lines
          $keepreading = 1;
          next;
          }
        else {
   ...
   if ($haveimage) {
      if (not ($bgimg =~ m/;/)) {print LOGFILE ("--> $bgimg\n  WARNING: background-image spec not properly closed\n");}
      $src = $bgimg; $src =~ s/^.*url\(['"]([^'" ]*)['"].*$/$1/i;
