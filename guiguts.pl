...
sub selection_menuitems {
	[
	   ...
	   [
		  ...
		  -command => sub {
			  $lglobal{surpop} =
				surround( $textwindow, $lglobal{surpop}, $top, $lglobal{font},
						  $activecolor, $icon );
			}
	   ],
	   [
				...
				if ( $errorchecktype eq 'W3C Validate' ) {
					...
					if ( $errorchecktype eq 'W3C Validate Remote' ) {
						...
						if ( $errorchecktype eq 'W3C Validate CSS' ) {
							...
							if ( $errorchecktype eq 'PPV Check' ) {
								errorcheckrun($errorchecktype);
							}
						}
						}
		...
		while ( $line = <$fh> ) {
			...
			if ( $errorchecktype eq 'HTML Tidy' ) {
					 ...
					 or ( $errorchecktype eq "W3C Validate Remote" ) )
				{
					...
					if (( $errorchecktype eq "W3C Validate CSS" )
					or ( $errorchecktype eq "PPV Check" )) {
						$line =~ s/^.*:(\d+:\d+)/line $1/;
						push @errorchecklines, $line;
						$errors{$line} = '';
						$lincol = '';
...
qq/java -jar $validatecsscommand file:$pwd\/$name > errors.err/ );
						...
						if ( $errorchecktype eq 'PPV Check' ) {
			system(qq/perl lib\/ppvchecks\/pphtml.pl -i $name -o errors.err/);
						}
					}
				}
			}
			...
			if ( $line =~ /Line (\d+)/ ) {
				...
				if ( $line =~ /Line \d+ column (\d+)/ ) {
					...
					if ( $line =~ /Query digit in ([\w\d]+)/ ) {
						$word = $1;
						$lincol =
						  $textwindow->search( '--', $word, "$linenum.0",
											   "$linenum.0 +1l" );
					}
...
# Highlight pretty close to GC error (2 chars before just in case error is at end of line)
	...
	sub gcviewops {
		...
		if ( defined( $lglobal{viewpop} ) ) {
			...
			$pframe->Label( -text => 'Select option to hide that error.', )
			  ->pack;
...
"$wordw words/phrases with markup, $suspects similar without. (\\n means newline)";
	...
	sub wfspellcheck {
		...
		$lglobal{saveheader} =
		  "$wordw words not recognised by the spellchecker.";
...
'R0lGODlhCAAQAIAAAAAAAP///yH5BAEAAAEALAAAAAAIABAAAAIUjAGmiMutopz0pPgwk7B6/3SZphQAOw==';
	...
	sub textbindings {
		...
		$textwindow->bind( 'TextUnicode',
						   '<Control-Shift-numbersign>' =>
							 sub { setbookmark( $textwindow, '3' ) } );
		$textwindow->bind( 'TextUnicode',
		...
		$textwindow->bind( 'TextUnicode', '<Shift-B1-Motion>',
						   'shiftB1_Motion' );
					...
					for my $page (@pages) {
						...
						if ( $pagetrack{$num}[3]->cget( -text ) eq 'Arabic' ) {
							$style = 'Arabic';
								 ...
								 $pagetrack{$num}[3]->cget( -text ) eq 'Roman' )
						{
							$style = 'Roman';
						}
						if ( $style eq 'Roman' ) {
...
### Search
	...
	sub gotoline {
		unless ( defined( $lglobal{gotolinepop} ) ) {
			...
			$lglobal{gotolinepop}->Subwidget('entry')
	...
	sub gotopage {
		unless ( defined( $lglobal{gotopagpop} ) ) {
			...
			$lglobal{gotopagpop}->Subwidget('entry')
			  ->selectionRange( 0, 'end' );
...
## Word Frequency
	sub wordcount {
		...
		if ( $lglobal{popup} ) {
			...
			$lglobal{wclistbox}
			  ->eventAdd( '<<search>>' => '<ButtonRelease-3>' );
...
## Sidenote Fixup
	sub sidenotes {
		...
		while (1) {
			...
			$bracketstartndx =
			  $textwindow->search( '-regexp', '--', '\[sidenote',
		...
		while (1) {
				 ...
				 "\n\n" )
			{
				if (
					(
					   $textwindow->get( $bracketendndx, "$bracketendndx+1c" )
					   eq ' '
					)
					|| ( $textwindow->get( $bracketendndx, "$bracketendndx+1c" )
						 eq "\n" )
...
## Find Greek
	sub findandextractgreek {
		...
		if ($closeIndex) {
			...
			$textwindow->tagAdd( 'highlight', $greekIndex,
								 $greekIndex . "+7c" );
			$textwindow->see('insert');
			$textwindow->tagAdd( 'highlight', $greekIndex,
								 $greekIndex . "+1c" );
	...
	sub setmargins {
		...
		if ( ( $blockrmargin < $blocklmargin ) || ( $rmargin < $lmargin ) ) {
			$top->messageBox(
						-icon => 'error',
						-message =>
						  'The left margins must come before the right margin.',
						-title => 'Incorrect margin ',
						-type  => 'OK',
			);
			setmargins();
...
"Single click right mouse button -- pop up shortcut to menu bar\n\n"
			...
			if ( $Tk::version ge 8.4 ) {
				...
				$tframe2->Button(
					...
					-command          => sub {
						...
						$lglobal{grtext}
						  ->insert( $start, togreektr($selection) );
	...
	sub latinpopup {
		if ( defined( $lglobal{latinpop} ) ) {
			...
			my $frame =
			  $lglobal{latinpop}->Frame( -background => 'white' )->pack;
			my @latinchars = (
							   [
								  '¿', '¡', '¬', '√',
								  'ƒ', '≈', '∆', '«'
