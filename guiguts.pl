...
sub Load{ # Custom File load routine; will automatically handle Unicode and line endings
        my ($w, $filename) = @_;
        ...
        if (open my $fh, '<', $filename){
                ...
                while (<$fh>){
                        ...
                        $w->ntinsert('end', $_);
                        if (($count++%1000) == 0){
                                $progress = $w->TextUndoFileProgress (Loading => $filename, $count, tell($fh), -s $filename);
                        }
                }
                close($fh);
...
sub SaveUTF{ # Custom file save routine to handle unicode files
        my ($w, $filename) = @_;
        $filename = $w->FileName unless defined $filename;
        ...
        unless ($perms & 0200){
                $perms = $perms | 0200;
                chmod $perms, $dir or $w->BackTrace("Can not write to directory $dir: $!\n") and return;
        }
        ...
        my ($lines) = $fileend =~ /^(\d+)\./;
        $unicode = $w->search('-regexp', '--', '[\x{100}-\x{FFFE}]', '1.0', 'end');
        if ($unicode){
                my $bom = "\x{FEFF}";
                utf8::encode($bom);
                print $tempfh $bom;
        ...
        if (rename ($tempfilename, $filename)){
                ...
                $w->BackTrace("Cannot save $filename:$!. Text is in the temporary file $tempfilename");
                return 0;
        }
}

sub IncludeFile{ #Custom File Include routine to handle Unicode and line ends
        my ($w, $filename) = @_;
        unless (defined($filename)){$w->BackTrace('filename not specified'); return;}
        if (open my $fh, '<', $filename){
                $w->Busy;
                my $count = 1;
                $w->addGlobStart;
                ...
                while (<$fh>){
                        ...
                        $w->insert('insert', $_);
                        if (($count++%1000) == 0){
                                $progress = $w->TextUndoFileProgress(Including => $filename, $count, tell($fh), -s $filename);
                        }
                }
                $progress->withdraw if defined $progress;
                $w->addGlobEnd;
...
sub SelectTo{ # Modified selection routine to deal with block selections
        ...
        if (($Tk::mouseMoved || $Tk::selectMode ne 'char') && $Tk::selectMode ne 'block'){
...
sub Column_Copy_or_Cut{ #modified Column Cut & Copy routine to handle block selection
        my ($w, $cut) = @_;
...
sub clipboardColumnPaste{ #modified Column Paste routine to handle block selection
        my ($w) = @_;
...
sub UpDownLine{
        ...
        }elsif ( ($n == 1) and ($ly + $lh) > ( $w->height - 2 * $w->cget(-bd) - 2 * $w->cget(-highlightthickness) - $lh + 1) ){
        ...
        return $i if ( $w->compare($newindex, '==', 'end - 1 char') and ($ny == $ly ) );
...
sub AutoScan{  # Modified to generate autoscroll events and accellerated scrolling.
        my ($w) = @_;
...
sub Populate {
        ...
        $rtext->bind( '<ButtonPress>', sub { $self->{'rtext'}->{'origx'} = undef;
                                                $self->_lineupdate }
        );
        ...
        if ( ref($rtext) eq 'TextUnicode' ) {
                push( @textMethods, 'Load', 'SaveUTF', 'IncludeFile', 'ntinsert', 'ntdelete', 'replacewith' );
        }
...
sub _lineupdate{
        my ($w) = @_;
        return unless ( $w->{'ltext'}->ismapped );      # Don't bother continuing if line numbers cannot be displayed
        ...
        while (1) {
                ...
                $oldy = $y;
                $ypix += $he;
                last if $ypix >= ($theight-1);  #we have reached the end of the display
                last if ( $y == $ypix );
...
our @extops = (
                ...
                {'label' => 'NPL Dictionary Search',                    'command' => 'start http://www.puzzlers.org/wordlists/grepdict.php'},
                {'label' => 'Pass open file to default handler','command' => 'start $d$f$e'},
                {'label' => '',                                 'command' => ''},
                {'label' => '',                                 'command' => ''},
                {'label' => '',                                 'command' => ''},
                {'label' => '',                                 'command' => ''},
...
$top->bind('<Configure>' => sub { # Detect geometry changes for tracking
                $geometry = $top->geometry;
                $lglobal{geometryupdate} = 1;
        }
);

my $icon = $top->Photo(-format => 'gif', -data => '
...
my $text_frame = $top->Frame->pack(-anchor => 'nw', -expand => 'yes', -fill => 'both'); # autosizing
my $counter_frame = $text_frame->Frame->pack(-side => 'bottom', -anchor => 'sw', -pady => 2, -expand => 0);
my $proofer_frame = $text_frame->Frame; # Frame to hold proofer names. Pack it when necessary.
...
$top->DropSite( -dropcommand => \&handleDND,
        -droptypes => (OS_Win or ($^O eq 'cygwin' and $Tk::platform eq 'MSWin32')) ? ['Win32'] : [qw/XDND Sun/]
);
...
$textwindow->SetGUICallbacks ([ # routines to call every time the text is edited
        \&update_indicators,
        sub{return if $nohighlights; $textwindow->HighlightAllPairsBracketingCursor},
        sub{$textwindow->hidelinenum unless $vislnnm}
]);
...
sub fileopen{           # Find a text file to open
        ...
        $name = $textwindow->getOpenFile(-filetypes => $types, -title => 'File Load', -initialdir => $globallastpath);
...
sub openfile{           # and open it
        ...
        return if (confirmempty() =~ /cancel/i);
        unless (-e $name){
                my $dbox = $top->Dialog(-text => 'Could not find file. Has it been moved or deleted?',
                        -bitmap => 'error', -title => 'Could not find File.', -buttons => ['Ok']);
                $dbox->Show;
                return;
        }
        clearvars();
        if ($lglobal{page_num_label}){$lglobal{page_num_label}->destroy; undef $lglobal{page_num_label}};
        if ($lglobal{page_label}){$lglobal{page_label}->destroy; undef $lglobal{page_label}};
        if ($lglobal{pagebutton}){$lglobal{pagebutton}->destroy; undef $lglobal{pagebutton}};
        if ($lglobal{proofbutton}){$lglobal{proofbutton}->destroy; undef $lglobal{proofbutton};}
        ...
        if (-e $binname){
                ...
                $textwindow->markSet('spellbkmk', $spellindexbkmrk) if $spellindexbkmrk;
...
sub prep_import{
        ...
        return 0 unless (-d $directory and defined $directory and $directory ne '');
        $top->Busy(-recurse => 1);
        my $pwd = getcwd();
        chdir $directory;
        my @files = glob "*.txt";
...
sub set_autosave{
        ...
        $lglobal{autosaveid} = $top->repeat(($autosaveinterval*60000), sub{ savefile()
                if $textwindow->numberChanges and $lglobal{global_filename} !~ /No File Loaded/});
        $lglobal{saveflashid} = $top->after(($autosaveinterval*60000 - 10000), sub{ flash_save()
                if $lglobal{global_filename} !~ /No File Loaded/ });
        $lglobal{savetool}->configure(-background => 'green', -activebackground => 'green') unless $notoolbar;
        $lglobal{autosaveinterval} = time;
}

sub flash_save {
        $lglobal{saveflashingid} = $top->repeat(500, sub {
                        if ($lglobal{savetool}->cget('-background') eq 'yellow') {
                                $lglobal{savetool}->configure(-background => 'green', -activebackground => 'green') unless $notoolbar;
                        }
                        else {
                                $lglobal{savetool}->configure(-background => 'yellow', -activebackground => 'yellow')
...
sub toggle_autosave {
        if ($autosave){
                ...
                $lglobal{savetool}->configure(-background => 'SystemButtonFace', -activebackground => 'SystemButtonFace') unless $notoolbar;
        }
}
                ...
                if($autobackup){
                        if (-e $lglobal{global_filename}){
                                ...
                                if (-e "$lglobal{global_filename}.bk1"){
                                        rename("$lglobal{global_filename}.bk1", "$lglobal{global_filename}.bk2");
                                }
                                rename($lglobal{global_filename}, "$lglobal{global_filename}.bk1");
                        }
                }
                $textwindow->SaveUTF;
        }
...
sub butbind{ # Bindings to make label in status bar act like buttons
        my $widget = shift;
        $widget->bind('<Enter>', sub{$widget->configure(-background => $activecolor); $widget->configure(-relief => 'raised')});
        $widget->bind('<Leave>', sub{$widget->configure(-background => 'gray'); $widget->configure(-relief => 'ridge')});
        $widget->bind('<ButtonRelease-1>', sub{$widget->configure(-relief => 'raised')});
}

sub paste{ # Special paste routine that will respond differently for overstrike/insert modes
...
sub utford{ # Pop up window to allow entering Unicode characters by ordinal number
        my $ord;
        my $base = 'dec';
        if ($lglobal{ordpop}){
                ...
                my $frame = $lglobal{ordpop}->Frame->pack(-fill => 'x', -padx =>5, -pady => 5);
                my $frame2 = $lglobal{ordpop}->Frame->pack(-fill => 'x', -padx => 5, -pady => 5);
                ...
                $inentry = $frame->Entry(
                        ...
                        -vcmd => sub{
                                        ...
                                        if ($base eq 'hex'){
                                                ...
                                                return 0 unless (($_[0] =~ /^\d{0,5}$/)&&($_[0] < 65519));
                                                $char = chr($_[0]);
                ...
                my $frame1 = $lglobal{ordpop}->Frame->pack(-fill => 'x', -padx => 5, -pady => 5);
                my $button = $frame1->Button(
                                        -text => 'OK',
                                        -width => 8,
                                        -command => sub {$lglobal{hasfocus}->insert('insert', $outentry->get('1.0', 'end -1c'))},
                )->grid(-row => 1, -column => 1);
                $frame1->Button(
                                        -text => 'Close',
                                        -width => 8,
                                        -command => sub {$lglobal{ordpop}->destroy; undef $lglobal{ordpop}},
                )->grid(-row => 1, -column => 2);
                $lglobal{ordpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{ordpop}->destroy; undef $lglobal{ordpop}});
                $lglobal{ordpop}->Icon(-image => $icon);
        }
}

sub selection{ # Pop up window allowing tracking and auto reselection of last selection
        my ($start, $end);
        if ($lglobal{selectionpop}){
                ...
                my $button = $frame1->Button(
                                        ...
                                        -width => 8,
                                        -command => sub {
                                                return unless (($start =~ /^\d+\.\d+$/) && ($end =~ /^\d+\.\d+$/));
                ...
                $lglobal{selectionpop}->protocol('WM_DELETE_WINDOW' => sub{
                                $lglobal{selectionpop}->destroy;
                                undef $lglobal{selectionpop};
                                undef $lglobal{selsentry};
                                undef $lglobal{seleentry}
...
sub update_indicators{ # Routine to update the status bar when somthing has changed.
        my ($last_line, $last_col) = split(/\./, $textwindow->index('end'));
        my ($line, $column) = split(/\./, $textwindow->index('insert'));
        $lglobal{current_line_label}->configure (-text => "Ln: $line/".($last_line-1)."  -  Col: $column");
        my $mode = $textwindow->OverstrikeMode;
        my $overstrke_insert =' I ';
        if ($mode)
                {$overstrke_insert =' O '}
        $lglobal{insert_overstrike_mode_label}->configure
                (-text => " $overstrke_insert ");
        my $filename = $textwindow->FileName;
        $filename = 'No File Loaded' unless(defined($filename));
        $lglobal{highlighlabel}->configure(-background => $highlightcolor) if ($lglobal{scanno_hl});
        $lglobal{highlighlabel}->configure(-background => 'gray') unless ($lglobal{scanno_hl});
        ...
        if ($lglobal{longordlabel}){
                ...
                $lglobal{ordinallabel}->configure(-text => " Dec $ordinal : Hex $hexi ", -width => 18);
        }
        ...
        if ($filename ne 'No File Loaded' or defined $lglobal{prepfile}){
                $lglobal{page_num_label}->configure(-text => 'Img: XXX') if defined $lglobal{page_num_label};
                ...
                while ($mark){
                        if($mark =~ /Pg(\w+)/){
                                ...
                                unless (defined($lglobal{page_num_label})){
                                        ...
                                        $lglobal{page_num_label}->bind('<1>', sub{
                                                $lglobal{page_num_label}->configure(-relief => 'sunken');
                                                gotopage();
                                                update_indicators();
                                                }
                                        );
                                        $lglobal{page_num_label}->bind('<3>', sub{
                                                $lglobal{page_num_label}->configure(-relief => 'sunken');
                                                viewpagenums();
                                                update_indicators();
                                                }
                                        ...
                                        $lglobal{statushelp}->attach($lglobal{page_num_label}, -balloonmsg => "Image/Page name for current page.");
                                }
                                unless (defined($lglobal{pagebutton})){
                                        ...
                                        $lglobal{pagebutton}->bind('<1>', sub{$lglobal{pagebutton}->configure(-relief => 'sunken'); openpng()});
                                        $lglobal{pagebutton}->bind('<3>', sub{setpngspath()});
                                        butbind($lglobal{pagebutton});
                                        $lglobal{statushelp}->attach($lglobal{pagebutton}, -balloonmsg => "Open Image corresponding to current page in an external viewer.");
                                }
                                unless ($lglobal{page_label}){
                                        ...
                                        $lglobal{page_label}->bind('<1>', sub{$lglobal{page_label}->configure(-relief => 'sunken'); gotolabel()});
                                        $lglobal{page_label}->bind('<3>', sub{$lglobal{page_label}->configure(-relief => 'sunken'); pageadjust()});
                                        $lglobal{statushelp}->attach($lglobal{page_label}, -balloonmsg => "Page label assigned to current page.");
                                }
                                $lglobal{page_num_label}->configure(-text => "Img: $pnum")if defined $lglobal{page_num_label};
                                ...
                                if ($textwindow->index('insert') > ($textwindow->index($mark) + 400)){
                                        last;
                                }
                ...
                if ((scalar %proofers)&&(defined($lglobal{pagebutton}))){
                        unless (defined($lglobal{proofbutton})){
                                        ...
                                        $lglobal{proofbutton}->bind('<1>', sub{$lglobal{proofbutton}->configure(-relief => 'sunken'); showproofers()});
                                        $lglobal{proofbutton}->bind('<3>', sub{$lglobal{proofbutton}->configure(-relief => 'sunken'); tglprfbar()});
                        ...
                        }
                        {
                                ...
                                for my $round ( 1..8 ) {
                                        ...
                                        $lglobal{proofbar}[$round]->configure(-text => "  Round $round  $proofers{$pnum}->[$round]  ") if $lglobal{proofbarvisible};
                                }
                        }
                }
        }
        $textwindow->tagRemove('bkmk', '1.0', 'end') unless $bkmkhl;
        if ($lglobal{geometryupdate}){saveset(); $lglobal{geometryupdate} = 0;};
        if ($autosave and $lglobal{autosaveinterval} and $DEBUG){
                my $elapsed = $autosaveinterval * 60 - (time - $lglobal{autosaveinterval});
...
sub toolbar_toggle{     # Set up / remove the tool bar
        if ($notoolbar&&$lglobal{toptool}){
                ...
                $lglobal{savetool}->bind('<Shift-3>', sub{
                                $autosave = !$autosave;
                                toggle_autosave();
                });
                $lglobal{toptool}->ToolButton(
                        -image => 'edittrash16',
                        -command => sub{return if (confirmempty() =~ /cancel/i); clearvars(); update_indicators()},
                        -tip => 'Discard Edits'
                );
                $lglobal{toptool}->separator;
                $lglobal{toptool}->ToolButton(
...
sub setcolor{   # Color picking routine
        my $initial = shift;
        return ($top->chooseColor(
                -initialcolor => $initial,
                -title => 'Choose color'
        ));
}

sub runner{     # Routine to spawn another perl process and use it to execute an external program
        my $args;
        $args = join ' ', @_;
        unless (-e 'spawn.pl'){
...
sub viewerpath{         #Find your image viewer
        my $types;
        if (OS_Win){$types = [['Executable', ['.exe', ]], ['All Files', ['*']],]}else{$types = [['All Files', ['*']]]};
        $lglobal{pathtemp} = $textwindow->getOpenFile(-filetypes => $types, -title => 'Where is your image viewer?', -initialdir => dirname($globalviewerpath));
...
sub setbrowser{         # Set up command to start a browser, varies by OS and browser
        ...
        $browsepop->Label(-text => "Enter the complete path to the executable.\n(Under Windows, you can use 'start' to use the default handler.\n".
        ...
        $browsepop->protocol('WM_DELETE_WINDOW' => sub{$browsepop->destroy; undef $browsepop;});
...
sub externalpopup{      # Set up the external commands menu
        ...
        if ($lglobal{xtpop}){
                ...
                for $menutempvar(0..9){
                        ...
                        )->grid(-row => "$menutempvar"+1, -column => 1, -padx => 2, -pady => 4);
                        $f1->Entry(
                                -width => 80,
                                -background => 'white',
                                -relief => 'sunken',
                                -textvariable => \$extops[$menutempvar]{command},
                        )->grid(-row => "$menutempvar"+1, -column => 2, -padx => 2, -pady => 4);
                };
                ...
                $lglobal{xtpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{xtpop}->destroy; undef $lglobal{xtpop}});
                $lglobal{xtpop}->Icon(-image => $icon);
        }
}

sub rebuildmenu{        # Menus are not easily modifiable in place. Easier to just destroy and rebuild every time it is modified
        for (0..10){
                $menu->delete('last');
        }
        buildmenu();
...
sub tglprfbar{  # Make toolbar visible if invisible and vice versa
        if ($lglobal{proofbarvisible}){
                ...
                $pnum =~ s/\D+//g;
                $proofer_frame->pack(-before => $counter_frame, -side => 'bottom', -anchor => 'sw', -expand => 0);
                my @geom = split /[x+]/, $top->geometry;
                $geom[1] += $counter_frame->height;
                $top->geometry("$geom[0]x$geom[1]+$geom[2]+$geom[3]");
                {
                        ...
                        for my $round ( 1..8 ) {
                                ...
                                $lglobal{proofbar}[$round]->bind('<1>' => sub{
...
sub highlightscannos{           #routine to automatically highlight words in the text
                        ...
                        for my $word(@words){
                                if (defined $lglobal{wordlist}->{$word}){
                                        ...
                                        while (1){
                                                ...
                                                if ($index > 0){
                                                        next if ($textwindow->get("$lglobal{hl_index}.@{[$index-1]}") =~ /\p{Alnum}/);
                                                }
                                                next if ($textwindow->get("$lglobal{hl_index}.@{[$index + length $word]}") =~ /\p{Alnum}/);
                                                $textwindow->tagAdd('scannos', "$lglobal{hl_index}.$index", "$lglobal{hl_index}.$index +@{[length $word]}c");
                                        }
                                }
                        }
                        $lglobal{hl_index}++;
        ...
        $textwindow->tagRemove('scannos', $idx1, $textwindow->index('@'.$textwindow->width.','.$textwindow->height));
                ...
                for my $word(@words){
                        if (defined $lglobal{wordlist}->{$word}){
                                ...
                                while (1){
                                        ...
                                        $indx = $index+length($word);
                                        if ($index > 0){
                                                next if ($textwindow->get("$realline.@{[$index - 1]}") =~ /\p{Alnum}/);
                                        }
                                        next if ($textwindow->get("$realline.@{[$index + length $word]}") =~ /\p{Alnum}/);
                                        $textwindow->tagAdd('scannos', "$realline.$index", "$realline.$index +@{[length $word]}c");
                                }
                        }
                }
                last unless defined $he;
...
sub about_pop_up{               # A litle information about the program
        if (defined($lglobal{aboutpop})){
                ...
                "Copyright 1999, 2003 - Greg London")->pack;
                my $button_ok = $lglobal{aboutpop}->Button(
                        -activebackground => $activecolor,
                        -text => 'OK',
                        -command => sub {$lglobal{aboutpop}->destroy; undef $lglobal{aboutpop}}
                )->pack(-pady => 6);
                $lglobal{aboutpop}->resizable('no', 'no');
                $lglobal{aboutpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{aboutpop}->destroy; undef $lglobal{aboutpop}});
...
sub buildmenu{ # The main menu building code.
        ...
        $menu->Cascade(-label => 'Sea~rch', -tearoff => 1, -menuitems => [
                        ...
                        [Button => 'Goto ~Line...', -command => sub{gotoline(); update_indicators();}],
                        [Button => 'Goto ~Page...', -command => sub{gotopage(); update_indicators();}],
                        [Button => '~Which Line?', -command => sub{$textwindow->WhatLineNumberPopUp}],
                        '',
                        [Button => 'Find next /*..*/ block', -command => [\&nextblock, 'default','forward']],
                        [Button => 'Find previous /*..*/ block', -command => [\&nextblock, 'default','reverse']],
                        [Button => 'Find next /#..#/ block', -command => [\&nextblock, 'block','forward']],
                        [Button => 'Find previous /#..#/ block', -command => [\&nextblock, 'block','reverse']],
                        [Button => 'Find next /$..$/ block', -command => [\&nextblock, 'stet','forward']],
                        [Button => 'Find previous /$..$/ block', -command => [\&nextblock, 'stet','reverse']],
                        [Button => 'Find next /p..p/ block', -command => [\&nextblock, 'poetry','forward']],
                        [Button => 'Find previous /p..p/ block', -command => [\&nextblock, 'poetry','reverse']],
                        [Button => 'Find next indented block', -command => [\&nextblock, 'indent','forward']],
                        [Button => 'Find previous indented block', -command => [\&nextblock, 'indent','reverse']],,
                        '',
                        [Button => 'Find ~Orphaned Brackets & Markup', -command => \&brackets],
                        '',
                        [Button => 'Highlight double quotes in selection', -command => [\&hilite, '"'],-accelerator => 'Ctrl+Shift+"'],
                        [Button => 'Highlight single quotes in selection', -command => [\&hilite, '\''],-accelerator => 'Ctrl+\''],
        ...
        $menu->Cascade(-label => '~Selection', -tearoff => 1, -menuitems => [
                        ...
                        [Button => '~UPPERCASE Selection', -command => sub{case('uc');}],
                        '',
                        [Button => 'Surround Selection With....', -command => \&surround],
                        [Button => 'Flood Fill Selection With....', -command => sub{$textwindow->addGlobStart; flood(); $textwindow->addGlobEnd;}],
                        '',
                        [Button => 'Indent Selection 1', -command => sub{$textwindow->addGlobStart; indent('in'); $textwindow->addGlobEnd;}],
                        [Button => 'Indent Selection -1', -command => sub{$textwindow->addGlobStart; indent('out'); $textwindow->addGlobEnd;}],
                        '',
                        [Button => '~Rewrap Selection', -command => sub{$textwindow->addGlobStart; selectrewrap(); $textwindow->addGlobEnd;}],
                        [Button => '~Block Rewrap Selection', -command => sub{$textwindow->addGlobStart; blockrewrap(); $textwindow->addGlobEnd;}],
                        [Button => 'Interrupt Rewrap', -command => sub{$operationinterrupt = 1}],
                        '',
                ...
                [
                        ...
                        [
                                [Button => 'Locate Gutcheck Executable', -command => sub{
                                        my $types;
                                        if (OS_Win){$types = [['Executable',['.exe',]],['All Files',['*']],]}else{$types = [['All Files',['*']]]};
                                        $lglobal{pathtemp} = $textwindow->getOpenFile(-filetypes => $types, -title => 'Where is the Gutcheck executable?', -initialdir => dirname($gutpath));
                                        $gutpath = $lglobal{pathtemp} if $lglobal{pathtemp};
                                        return unless $gutpath;
                                        $gutpath = os_normal($gutpath);
                                        saveset();
                                        }
                                ],
                                [Button => 'Locate Jeebies Executable', -command => sub{
                                        my $types;
                                        if (OS_Win){$types = [['Executable',['.exe',]],['All Files',['*']],]}else{$types = [['All Files',['*']]]};
                                        $lglobal{pathtemp} = $textwindow->getOpenFile(-filetypes => $types, -title => 'Where is the Jeebies executable?', -initialdir => dirname($jeebiespath));
                                        $jeebiespath = $lglobal{pathtemp} if $lglobal{pathtemp};
                                        return unless $jeebiespath;
                                        $jeebiespath = os_normal($jeebiespath);
                                        saveset();
                                        }
                                ],
                                [Button => 'Locate Aspell Executable', -command => sub{
                                        my $types;
                                        if (OS_Win){$types = [['Executable',['.exe',]],['All Files',['*']],]}else{$types = [['All Files',['*']]]};
                                        $lglobal{pathtemp} = $textwindow->getOpenFile(-filetypes => $types, -title => 'Where is the Aspell executable?', -initialdir => dirname($globalspellpath));
                                        $globalspellpath = $lglobal{pathtemp} if $lglobal{pathtemp};
                                        return unless $globalspellpath;
                                        $globalspellpath = os_normal($globalspellpath);
                                        saveset();
                                        }
                                ],
                                [Button => 'Locate Tidy Executable', -command => sub{
                                        my $types;
                                        if (OS_Win){$types = [['Executable',['.exe',]],['All Files',['*']],]}else{$types = [['All Files',['*']]]};
                                        $tidycommand = $textwindow->getOpenFile(-filetypes => $types, -title => 'Where is the Tidy executable?');
                                        return unless $tidycommand;
                                        $tidycommand = os_normal($tidycommand);
                                        saveset();
                                        }
                                ],
                                [Button => 'Locate Image Viewer Executable', -command => \&viewerpath],
                        ...
                        [Cascade => 'Toolbar Prefs', -tearoff => 1, -menuitems => 
                                [
                                        ...
                                        [Radiobutton => 'Toolbar on Right', -variable => \$toolside, -command => sub{$lglobal{toptool}->destroy if $lglobal{toptool}; undef $lglobal{toptool}; toolbar_toggle()}, -value => 'right'],
                                ]
                        ],
                        [Button => 'Set Button Highlight Color', -command => sub{
                                                                                        my $thiscolor = setcolor($activecolor);
                                                                                        $activecolor = $thiscolor if $thiscolor;
                                                                                        OS_Win ? $lglobal{checkcolor} = 'white' : $lglobal{checkcolor} = $activecolor;
                                                                                }
                        ],
                        ...
                        [Button => 'Auto Save Interval', -command => sub{
                                                                        saveinterval();
                                                                        saveset();
                                                                        set_autosave() if $autosave;
                                                                }
        ...
        $menu->Cascade(-label => '~Help', -tearoff => 1, -menuitems => 
                [
                        ...
                        [Button => '~Manual', -command => sub{runner("$globalbrowserstart guiguts.html") if (-e 'guiguts.html');}
                        ],
        ...
        if ($Tk::version ge 8.4){
                ...
                if ($lglobal{utfrangesort}){
                        ...
                        [       [Radiobutton => 'Sort by Name',
                                ...
                                map (
                                        ...
                                        -command => [\&utfpopup, $utfsorthash{$_}, $lglobal{utfblocks}{$utfsorthash{$_}}[0], $lglobal{utfblocks}{$utfsorthash{$_}}[1]],
                                        -accelerator => $lglobal{utfblocks}{$utfsorthash{$_}}[0].' - '.$lglobal{utfblocks}{$utfsorthash{$_}}[1]],
                ...
                $menu->Cascade(qw/-label Unicode -tearoff 0 -menuitems/ => 
                        [       [Radiobutton => 'Sort by Range',
                                ...
                                map (
                                        [Button  => "$_",
                                        -command => [\&utfpopup, $_, $lglobal{utfblocks}{$_}[0], $lglobal{utfblocks}{$_}[1]],
        ...
        if ($DEBUG){
                        $menu->Command(-label => 'Test', -command => sub{
                        });
        }
}
...
sub viewpagenums{ # Toggle visible page markers
        if ($lglobal{seepagenums}){
                ...
                if ($lglobal{pnumpop}){$lglobal{pnpopgoem} = $lglobal{pnumpop}->geometry; $lglobal{pnumpop}->destroy; undef $lglobal{pnumpop};};
        }else{
...
sub gotopage{ # Pop up a window which will allow jumping directly to a specified page
        unless (defined($lglobal{gotopagpop})){
                ...
                $lglobal{gotopagpop} = $top->DialogBox(
                        -buttons => [qw[Ok Cancel]],-title => 'Goto Page Number',
                        -popover => $top,
                        -command => sub {
                                if ($_[0] eq 'Ok'){
                                        ...
                                        if ($lglobal{pagedigits} == 3){
                                                $lglobal{lastpage} = sprintf("%03s",$lglobal{lastpage});
                                        }elsif ($lglobal{pagedigits} == 4){
                                                $lglobal{lastpage} = sprintf("%04s",$lglobal{lastpage});
                                        }
                                        unless (exists $pagenumbers{'Pg'.$lglobal{lastpage}} && defined $pagenumbers{'Pg'.$lglobal{lastpage}}){
                ...
                $frame->Label(-text => 'Enter image number: ')->pack(-side => 'left');
...
sub gotolabel{ # Pop up a window which will allow jumping directly to a specified page
        unless (defined($lglobal{gotolabpop})){
                ...
                $lglobal{gotolabpop} = $top->DialogBox(
                        -buttons => [qw[Ok Cancel]],-title => 'Goto Page Label',
                        -popover => $top,
                        -command => sub {
                                if ($_[0] eq 'Ok'){
                                        my $mark;
                                        for (keys %pagenumbers){
                                                if ($pagenumbers{$_}{label} && $pagenumbers{$_}{label} eq $lglobal{lastlabel}){
                                                        $mark = $_;
                                                        last;
                                                }
                                        }
...
sub saveinterval{ # Pop up a window where you can adjust the auto save interval
        if ($lglobal{intervalpop}){
                ...
                $frame->Label(-text => 'Minutes between Autosave')->pack(-side => 'left');
                my $entry = $frame->Entry(
                        -background => 'white',
                        -width => 5,
                ...
                $lglobal{intervalpop}->protocol('WM_DELETE_WINDOW' => sub{$autosaveinterval = 5 unless $autosaveinterval; $lglobal{intervalpop}->destroy; undef $lglobal{intervalpop}});
...
sub opspop_up{ # Pop up an "Operation" history. Track which functions have already been run.
        if ($lglobal{oppop}){
                ...
                my $frame = $lglobal{oppop}->Frame->pack(-anchor => 'nw', -fill => 'both', -expand => 'both', -padx => 2, -pady => 2);
                $lglobal{oplistbox} = $frame->Scrolled('Listbox',
                        -scrollbars => 'se',
                        -background => 'white',
                        -selectmode => 'single',
                        -activestyle => 'none',
                ...
                $lglobal{oppop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{oppop}->destroy; undef $lglobal{oppop}});
                $lglobal{oppop}->Icon(-image => $icon);
        }
        oppopupdate();
        $lglobal{oppop}->bind('<Configure>' => sub {
...
sub footnotepop{ # Pop up a widow where footnotes can be found, fixed and formatted. (heh)
        ...
        if (defined($lglobal{footpop})){
                ...
                $frame2->Button(
                        -activebackground => $activecolor,
                        -command => sub {       $textwindow->yview('end');
                                                $textwindow->see($lglobal{fnarray}->[$lglobal{fnindex}][2]) if $lglobal{fnarray}->[$lglobal{fnindex}][2];
                                        },
                        -text => 'See Anchor',
                        -width => 14
                ...
                $lglobal{fnindexbrowse} = $frame2->BrowseEntry(
                        ...
                        -listwidth => 22,
                        -browsecmd => sub{
                                                $lglobal{fnindex} = $lglobal{fntotal} if ($lglobal{fnindex} > $lglobal{fntotal});
                ...
                my $fnrb2 = $frame2->Radiobutton(
                        ...
                        -command => sub{
                                                $lglobal{fnindex} =1;
                                                footnoteshow();
                                                $lglobal{fnmvbutton}->configure(-state => 'normal') if ($lglobal{fnsecondpass}&&(defined $lglobal{fnlzs} and @{$lglobal{fnlzs}}));
                                        },
                )->grid(-row => 8, -column => 3);
                ...
                $frame1->Button(
                        ...
                        -command => sub {
                                                ...
                                                if ($lglobal{fnlzs}[$lglobal{zoneindex}]){
                                                        ...
                                                        $textwindow->tagAdd('highlight','LZ'.$lglobal{zoneindex},'LZ'.$lglobal{zoneindex}.'+10c')
                                                }
                                        },
                        -text => '<--- Last LZ',
                        -width => 12
                ...
                $frame1->Button(
                        ...
                        -command => sub {
                                                getlz();
                                                return unless $lglobal{fnlzs} and @{$lglobal{fnlzs}};
                                                $lglobal{zoneindex}++ unless $lglobal{zoneindex} > ((scalar(@{$lglobal{fnlzs}}))-2);
                                                if ($lglobal{fnlzs}[$lglobal{zoneindex}]){
                                                        $textwindow->see('LZ'.$lglobal{zoneindex});
                                                        $textwindow->tagRemove('highlight','1.0','end');
                                                        $textwindow->tagAdd('highlight','LZ'.$lglobal{zoneindex},'LZ'.$lglobal{zoneindex}.'+10c')
                ...
                $lglobal{footpop}->protocol('WM_DELETE_WINDOW' => sub{
...
sub fnview{ # Pop up a window showing all the footnote addresses with potential problems highlighted
        ...
        if (defined($lglobal{footviewpop})){
                ...
                my $frame1 = $lglobal{footviewpop}->Frame(
                        -background => 'white'
                ...
                my $frame2 = $lglobal{footviewpop}->Frame->pack(-side => 'top', -anchor => 'n', -fill => 'both', -expand => 'both');
                $ftext = $frame2->Scrolled('ROText',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => $lglobal{font},
                )->pack(-anchor => 'nw', -fill => 'both', -expand => 'both', -padx => 2, -pady => 2);
                drag($ftext);
                ...
                $lglobal{footviewpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{footviewpop}->destroy; undef $lglobal{footviewpop}});
                $lglobal{footviewpop}->Icon(-image => $icon);
                for my $findex(1..$lglobal{fntotal}){
                        $ftext->insert('end','footnote #'.$findex.'  line.column - '.$lglobal{fnarray}->[$findex][0].",\tanchor line.column - ".$lglobal{fnarray}->[$findex][2]."\n");
                        if ($lglobal{fnarray}->[$findex][0] eq $lglobal{fnarray}->[$findex][2]){
                                $ftext->tagAdd('noanch','end -2l','end -1l');
                                $ftext->update;
                        }
                        if (($findex > 1) && ($textwindow->compare($lglobal{fnarray}->[$findex][0] ,'<', $lglobal{fnarray}->[$findex-1][0])||$textwindow->compare($lglobal{fnarray}->[$findex][2] ,'<', $lglobal{fnarray}->[$findex-1][2]))){
...
sub footnotetidy{ # Clean up footnotes in ASCII version of text. Note: destructive. Use only at end of editing.
        my ($begin, $end, $colon);
        $lglobal{fnsecondpass} = 0;
        footnotefixup();
        ...
        while(1){
                ...
                $colon = $textwindow->search('--',':',$begin,$textwindow->index('fne'.$lglobal{fnindex}));
...
sub footnotemove{
        ...
        foreach $lz(@{$lglobal{fnlzs}}){
                if ($lglobal{fnarray}->[$lglobal{fnindex}][0]){
                        while ($textwindow->compare($lglobal{fnarray}->[$lglobal{fnindex}][0],'<=',$lz)){
        ...
        while ($lglobal{fnindex}){
                $textwindow->delete("fne$lglobal{fnindex} +1c") if ($textwindow->get("fne$lglobal{fnindex} +1c") eq "\n");
                $textwindow->delete("fns$lglobal{fnindex} -1c") if ($textwindow->get("fns$lglobal{fnindex} -1c") eq "\n");
                $textwindow->delete("fns$lglobal{fnindex}","fne$lglobal{fnindex}");
                $lglobal{fnindex}--;
        }
        $zone = 0;
        foreach $lz(@{$lglobal{fnlzs}}){
                $textwindow->insert($textwindow->index("LZ$zone +10c"), $footnotes{$lz}) if $footnotes{$lz};
                $footnotes{$lz} ='';
                $zone++;
        }
        $zone = 1;
        ...
        while (1){
                ...
                last unless ($index);
                unless ($textwindow->get("$index +2l") =~ /^\[/){$textwindow->delete($index,"$index+12c")}
                $index .= '+4l';
        }
        $textwindow->markSet('insert','1.0');
        $textwindow->see('1.0');
...
sub setlz{
        ...
        $lglobal{fnmvbutton}->configure('-state' => 'normal') if (($lglobal{fnsecondpass})&&($lglobal{footstyle} eq 'end'));
}
...
sub setanchor{
        ...
        if ($lglobal{fnarray}->[$lglobal{fnindex}][0] ne $lglobal{fnarray}->[$lglobal{fnindex}][2]){
                $textwindow->delete($lglobal{fnarray}->[$lglobal{fnindex}][2],$lglobal{fnarray}->[$lglobal{fnindex}][3]) if $lglobal{fnarray}->[$lglobal{fnindex}][3];
        }else{
        ...
        if ($lglobal{footstyle} eq 'inline'){
                ...
                footnoteadjust();
                my $fn = $textwindow->get($textwindow->index('fns'.$lglobal{fnindex}),$textwindow->index('fne'.$lglobal{fnindex}));
                $textwindow->insert($textwindow->index("fna$lglobal{fnindex}"),$fn) if $textwindow->compare($textwindow->index("fna$lglobal{fnindex}"),'>',$textwindow->index("fns$lglobal{fnindex}"));
                $textwindow->delete($textwindow->index("fns$lglobal{fnindex}"),$textwindow->index("fne$lglobal{fnindex}"));
                $textwindow->insert($textwindow->index("fna$lglobal{fnindex}"),$fn) if $textwindow->compare($textwindow->index("fna$lglobal{fnindex}"),'<=',$textwindow->index("fns$lglobal{fnindex}"));
                $lglobal{fnarray}->[$lglobal{fnindex}][0] = $textwindow->index('fns'.$lglobal{fnindex});
                ...
                $lglobal{fnarray}->[$lglobal{fnindex}][2] = $insert;
                if ($textwindow->compare($lglobal{fnarray}->[$lglobal{fnindex}][2],'>',$lglobal{fnarray}->[$lglobal{fnindex}][0])){$lglobal{fnarray}->[$lglobal{fnindex}][2] = $lglobal{fnarray}->[$lglobal{fnindex}][0]};
                $textwindow->insert($lglobal{fnarray}->[$lglobal{fnindex}][2],'['.$lglobal{fnarray}->[$lglobal{fnindex}][4].']');
                $textwindow->update;
                $lglobal{fnarray}->[$lglobal{fnindex}][3] = $textwindow->index($lglobal{fnarray}->[$lglobal{fnindex}][2].'+'.(length($lglobal{fnarray}->[$lglobal{fnindex}][4])+2).'c');
                $textwindow->markSet("fna$lglobal{fnindex}",$lglobal{fnarray}->[$lglobal{fnindex}][2]);
                $textwindow->markSet("fnb$lglobal{fnindex}",$lglobal{fnarray}->[$lglobal{fnindex}][3]);
                footnoteadjust();
                footnoteshow();
        }
}
...
sub footnotefixup{
        ...
        $lglobal{footnotenumber}->configure(-text => $lglobal{fncount}) if $lglobal{footpop};
        $lglobal{footnoteletter}->configure(-text => alpha($lglobal{fnalpha})) if $lglobal{footpop};
        $lglobal{footnoteroman}->configure(-text => roman($lglobal{fnroman})) if $lglobal{footpop};
        ...
        $textwindow->tagRemove('highlight','1.0','end');
        while(1){
                $lglobal{ftnoteindexstart} = $textwindow->search('-exact','--','[ Footnote','1.0','end');
        ...
        while(1){
                $lglobal{ftnoteindexstart} = $textwindow->search('-exact','--','[Fotonote','1.0','end');
                last unless $lglobal{ftnoteindexstart};
        ...
        while(1){
                $lglobal{ftnoteindexstart} = $textwindow->search('-exact','--','[Footnoto','1.0','end');
                last unless $lglobal{ftnoteindexstart};
                $textwindow->delete("$lglobal{ftnoteindexstart}+1c","$lglobal{ftnoteindexstart}+9c");
                $textwindow->insert("$lglobal{ftnoteindexstart}+1c",'Footnote');
        }
        ...
        while (1){
                ...
                $lglobal{fntotal}++;
                $lglobal{fnindex} = $lglobal{fntotal};
                ($start,$end) = ($textwindow->index("fns$lglobal{fnindex}"),$textwindow->index("fne$lglobal{fnindex}")) if $lglobal{fnsecondpass};
                $pointer = '';
                $anchor = '';
                ...
                $lglobal{fnindexbrowse}->insert('end',$lglobal{fnindex}) if $lglobal{footpop};
                $lglobal{footnotetotal}->configure(-text => "# $lglobal{fnindex}/$lglobal{fntotal}") if $lglobal{footpop};
                $pointer = $textwindow->get($start,($textwindow->search('--',':',$start,"$start lineend")));
                $pointer =~ s/\[Footnote\s*//i;
                $pointer =~ s/\s*:$//;
                if (length($pointer) > 20){$pointer = ''; $textwindow->insert("$start+9c",':')};
                if ($lglobal{fnsearchlimit}){
                        $anchor = $textwindow->search('-backwards','--',"[$pointer]",$start,'1.0') if $pointer;
                        #print "$pointer|$anchor\n";
                }else{
                        $anchor = $textwindow->search('-backwards','--',"[$pointer]",$start,"$start-80l") if $pointer;
                }
                $textwindow->tagAdd('highlight',$anchor,$anchor.'+'.(length($pointer)+2).'c') if $anchor;
                $lglobal{fnarray}->[$lglobal{fnindex}][0] = $start if $start;
                $lglobal{fnarray}->[$lglobal{fnindex}][1] = $end if $end;
                $lglobal{fnarray}->[$lglobal{fnindex}][2] = $start unless ($pointer&&$anchor);
                $lglobal{fnarray}->[$lglobal{fnindex}][2] = $anchor if $anchor;
                $lglobal{fnarray}->[$lglobal{fnindex}][3] = $start unless ($pointer&&$anchor);
                $lglobal{fnarray}->[$lglobal{fnindex}][3] = $textwindow->index($lglobal{fnarray}->[$lglobal{fnindex}][2].'+'.(length($pointer)+2).'c') if $anchor;
                $lglobal{fnarray}->[$lglobal{fnindex}][4] = $pointer if $pointer;
                if ($pointer){
                        $lglobal{fnarray}->[$lglobal{fnindex}][5] = 'n';
                        if ($pointer =~ /\p{IsAlpha}+/){$lglobal{fnarray}->[$lglobal{fnindex}][5] = 'a'; $lglobal{fnarray}->[$lglobal{fnindex}][4] = uc($pointer)};
                        if ($pointer =~ /[ivxlcdm]+\./i){$lglobal{fnarray}->[$lglobal{fnindex}][5] = 'r'; $lglobal{fnarray}->[$lglobal{fnindex}][4] = uc($pointer)};
                }else{
                ...
                if ($lglobal{fnsecondpass}){
                        if ($lglobal{footstyle} eq 'end'){
                                ...
                                fninsertmarkers('n') if (($lglobal{fnarray}->[$lglobal{fnindex}][5] eq 'n')||($lglobal{fnarray}->[$lglobal{fnindex}][5] eq '')||($lglobal{fntypen}));
                                fninsertmarkers('a') if (($lglobal{fnarray}->[$lglobal{fnindex}][5] eq 'a')||($lglobal{fntypea}));
                                fninsertmarkers('r') if (($lglobal{fnarray}->[$lglobal{fnindex}][5] eq 'r')||($lglobal{fntyper}));
                                $lglobal{fnmvbutton}->configure('-state' => 'normal') if (defined $lglobal{fnlzs} and @{$lglobal{fnlzs}});
                        }else{
        ...
        $lglobal{fnfpbutton}->configure('-state' => 'normal') if $lglobal{footpop};
        footnoteshow();
}
...
sub footnoteshow{
        ...
        if ($lglobal{fncenter}){
                ...
                my $bottom = int(($widget->height - 2*$widget->cget(-bd) - 2*$widget->cget(-highlightthickness))/$lh/2)-1;
        ...
        $textwindow->tagAdd('highlight',$anchor,$anchorend) if (($anchor ne $start)&&$anchorend);
        $lglobal{footnotetotal}->configure(-text => "# $lglobal{fnindex}/$lglobal{fntotal}") if $lglobal{footpop};
        update_indicators();
}
...
sub fninsertmarkers{
        my $style = shift;
        my $offset = $textwindow->search('--',':',$lglobal{fnarray}->[$lglobal{fnindex}][0],$lglobal{fnarray}->[$lglobal{fnindex}][1]);
        if ($lglobal{footstyle} eq 'end'){
                $textwindow->delete($lglobal{fnarray}->[$lglobal{fnindex}][0].'+9c',$offset) if $offset;
                if ($lglobal{fnarray}->[$lglobal{fnindex}][3] ne $lglobal{fnarray}->[$lglobal{fnindex}][2]){
                        $textwindow->delete($lglobal{fnarray}->[$lglobal{fnindex}][2],$lglobal{fnarray}->[$lglobal{fnindex}][3]);
                }
                $lglobal{fnarray}->[$lglobal{fnindex}][6] = $lglobal{fncount} if $style eq 'n';
                $lglobal{fnarray}->[$lglobal{fnindex}][6] = $lglobal{fnalpha} if $style eq 'a';
                $lglobal{fnarray}->[$lglobal{fnindex}][6] = $lglobal{fnroman} if $style eq 'r';
                $lglobal{fnarray}->[$lglobal{fnindex}][5] = $style;
                $lglobal{fnarray}->[$lglobal{fnindex}][4] = $lglobal{fncount} if $style eq 'n';
                $lglobal{fnarray}->[$lglobal{fnindex}][4] = alpha($lglobal{fnalpha}) if $style eq 'a';
                $lglobal{fnarray}->[$lglobal{fnindex}][4] = roman($lglobal{fnroman}) if $style eq 'r';
                $lglobal{fncount}++ if $style eq 'n';
                $lglobal{fnalpha}++ if $style eq 'a';
                $lglobal{fnroman}++ if $style eq 'r';
                footnoteadjust();
                $textwindow->insert($lglobal{fnarray}->[$lglobal{fnindex}][0].'+9c',' '.$lglobal{fnarray}->[$lglobal{fnindex}][4]);
                $textwindow->insert($lglobal{fnarray}->[$lglobal{fnindex}][2],'['.$lglobal{fnarray}->[$lglobal{fnindex}][4].']');
                $lglobal{fnarray}->[$lglobal{fnindex}][3] = $textwindow->index($lglobal{fnarray}->[$lglobal{fnindex}][2].' +'.(length($lglobal{fnarray}->[$lglobal{fnindex}][4])+2).'c');
                $textwindow->markSet("fna$lglobal{fnindex}",$lglobal{fnarray}->[$lglobal{fnindex}][2]);
                $textwindow->markSet("fnb$lglobal{fnindex}",$lglobal{fnarray}->[$lglobal{fnindex}][3]);
...
sub fnjoin{
        $textwindow->tagRemove('footnote','1.0','end');
        $textwindow->tagRemove('highlight','1.0','end');
        my $start = $textwindow->search('--',':',$lglobal{fnarray}->[$lglobal{fnindex}][0],$lglobal{fnarray}->[$lglobal{fnindex}][1]);
        my $end = $lglobal{fnarray}->[$lglobal{fnindex}][1].'-1c';
        $textwindow->delete($lglobal{fnarray}->[$lglobal{fnindex}-1][1]) if ($textwindow->get($lglobal{fnarray}->[$lglobal{fnindex}-1][1]) eq '*');
        ...
        $textwindow->delete("fns$lglobal{fnindex}-1c","fns$lglobal{fnindex}") if ($textwindow->get("fns$lglobal{fnindex}-1c") eq '*');
...
sub footnoteadjust{
        ...
        unless ($start and $lglobal{fnindex}){
                ...
                while ($lglobal{fnarray}->[$lglobal{fnindex}+1][0]){
                        $lglobal{fnarray}->[$lglobal{fnindex}][0] = $textwindow->index('fns'.($lglobal{fnindex}+1));
                        $textwindow->markSet("fns$lglobal{fnindex}",$lglobal{fnarray}->[$lglobal{fnindex}][0]);
                        $lglobal{fnarray}->[$lglobal{fnindex}][1] = $textwindow->index('fne'.($lglobal{fnindex}+1));
                        $textwindow->markSet("fne$lglobal{fnindex}",$lglobal{fnarray}->[$lglobal{fnindex}][1]);
                        $lglobal{fnarray}->[$lglobal{fnindex}][2] = $textwindow->index('fna'.($lglobal{fnindex}+1));
                        $textwindow->markSet("fna$lglobal{fnindex}",$lglobal{fnarray}->[$lglobal{fnindex}][2]);
                        $lglobal{fnarray}->[$lglobal{fnindex}][3] = '';
                        $lglobal{fnarray}->[$lglobal{fnindex}][3] = $textwindow->index('fnb'.($lglobal{fnindex}+1)) if $lglobal{fnarray}->[$lglobal{fnindex}+1][3];
                        $textwindow->markSet("fnb$lglobal{fnindex}",$lglobal{fnarray}->[$lglobal{fnindex}][3]) if $lglobal{fnarray}->[$lglobal{fnindex}+1][3];
                        $lglobal{fnarray}->[$lglobal{fnindex}][4] = $lglobal{fnarray}->[$lglobal{fnindex}+1][4];
                        $lglobal{fnarray}->[$lglobal{fnindex}][5] = $lglobal{fnarray}->[$lglobal{fnindex}+1][5];
                        $lglobal{fnarray}->[$lglobal{fnindex}][6] = $lglobal{fnarray}->[$lglobal{fnindex}+1][6];
                        $lglobal{fnindex}++;
                }
                $lglobal{footnotenumber}->configure(-text => $lglobal{fncount});
                $lglobal{footnoteletter}->configure(-text => alpha($lglobal{fnalpha}));
        ...
        if ($lglobal{fnindex} > 1){
                $lglobal{ftnoteindexstart} = $lglobal{fnarray}->[$lglobal{fnindex}-1][1];
                $textwindow->markSet('fnindex',$lglobal{ftnoteindexstart});
        }else{
                $lglobal{ftnoteindexstart} = '1.0';
                $textwindow->markSet('fnindex',$lglobal{ftnoteindexstart});
...
sub footnotefind{
        ...
        $bracketstartndx = $textwindow->search('-regexp','--','\[[Ff][Oo][Oo][Tt]',$lglobal{ftnoteindexstart},'end');
...
sub roman {     # Roman numeral conversion taken directly from the Roman.pm module Copyright (c) 1995 OZAWA Sakuro. Done to avoid users
        my %roman_digit = qw(1 IV 10 XL 100 CD 1000 MMMMMM);    # having to install downloadable modules.
...
sub fontsize{
        ...
        if (defined($lglobal{fspop})){
                ...
                my $fontlist = $tframe->BrowseEntry(
                        -label => 'Font',
                        -browsecmd => sub {fontinit(); $textwindow->configure(-font => $lglobal{font})},
                        -variable => \$fontname
                ...
                $sizelabel = $mframe->Label(-text => $fontsize)->grid(-row => 1, -column => 2, -pady => 5);
                ...
                my $weightbox = $mframe->Checkbutton(
                        ...
                        -command => sub{fontinit(); $textwindow->configure(-font => $lglobal{font})},
                        -text => 'Bold'
                )->grid(-row => 2, -column => 2, -pady => 5);
                my $button_ok = $mframe->Button(
                        -activebackground => $activecolor,
                        -text => 'OK',
                        -command => sub {$lglobal{fspop}->destroy; undef $lglobal{fspop}; saveset()}
                )->grid(-row => 3, -column => 2, -pady => 5);
                $lglobal{fspop}->resizable('no','no');
                $lglobal{fspop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{fspop}->destroy; undef $lglobal{fspop}});
...
sub spellchecker{                                               # Set up spell check window
        ...
        if (defined($lglobal{spellpopup})){                     # If window already exists
                ...
                spelloptions() unless $globalspellpath;         # Whoops, don't know where to find Aspell
                spellclearvars();
                spellcheckfirst();                              # Start checking the spelling
        }else{                                                  # window doesn't exist so set it up
                $lglobal{spellpopup} = $top->Toplevel;
                $lglobal{spellpopup}->title('Current Dictionary - '.$globalspelldictopt||'<default>');
                my $spf1 = $lglobal{spellpopup}->Frame->pack(-side => 'top', -anchor => 'n', -padx => 5);
                $lglobal{misspelledlabel} = $spf1->Label(
                ...
                $lglobal{suggestionlabel} = $spf1->Label(
                        -text => 'Suggestions:',
                        )->pack(-side => 'top', -anchor => 'n', -pady => 5);
                ...
                my $spf2 = $lglobal{spellpopup}->Frame->pack(-side => 'top', -anchor => 'n', -padx => 5);
                my $changebutton = $spf2->Button(
                        -activebackground => $activecolor,
                        -command => sub {spellreplace()},
                        -text => 'Change',
                        -width => 14
                )->pack(-side => 'left', -pady => 2, -padx => 3, -anchor => 'nw');
                my $ignorebutton = $spf2->Button(
                        -activebackground => $activecolor,
                        -command => sub {shift @{$lglobal{misspelledlist}}; spellchecknext()},
                        -text => 'Skip <Ctrl+s>',
                        -width => 14
                )->pack(-side => 'left', -pady => 2, -padx => 3, -anchor => 'nw');
                ...
                $spf2->Button(
                        -activebackground => $activecolor,
                        -command => sub {$spellindexbkmrk = $textwindow->index($lglobal{lastmatchindex}.'-1c')||'1.0';
                ...
                my $spf3 = $lglobal{spellpopup}->Frame->pack(-side => 'top', -anchor => 'n', -padx => 5);
                ...
                my $spf4 = $lglobal{spellpopup}->Frame->pack(-side => 'top', -anchor => 'n', -padx => 5);
                my $dictaddbutton = $spf4->Button(
                        -activebackground => $activecolor,
                        -command => sub {spelladdword(); spellignoreall(); spellchecknext()},
                        -text => 'Add To Aspell Dic. <Ctrl+a>',
                        -width => 22,
                )->pack(-side => 'left', -pady => 2, -padx => 3, -anchor => 'nw');
                my $dictmyaddbutton = $spf4->Button(
                        -activebackground => $activecolor,
                        -command => sub {spellmyaddword($lglobal{misspelledentry}->get); spellignoreall(); spellchecknext()},
                        -text => 'Add To Project Dic. <Ctrl+p>',
                        -width => 22,
                )->pack(-side => 'left', -pady => 2, -padx => 3, -anchor => 'nw');
                $lglobal{spellpopup}->protocol('WM_DELETE_WINDOW' => sub{
                                                                @{$lglobal{misspelledlist}} = ();
                                                                $lglobal{spellpopup}->destroy;
                                                                undef $lglobal{spellpopup};     # completly remove spellcheck window
                ...
                $lglobal{spellpopup}->bind('<Control-a>',sub {$lglobal{spellpopup}->focus; spelladdword(); spellignoreall(); spellchecknext()});
                $lglobal{spellpopup}->bind('<Control-p>',sub {$lglobal{spellpopup}->focus; spellmyaddword($lglobal{misspelledentry}->get); spellignoreall(); spellchecknext()});
                $lglobal{spellpopup}->bind('<Control-s>',sub {$lglobal{spellpopup}->focus; shift @{$lglobal{misspelledlist}}; spellchecknext()});
                $lglobal{spellpopup}->bind('<Control-i>',sub {$lglobal{spellpopup}->focus; spellignoreall(); spellchecknext()});
                $lglobal{spellpopup}->bind('<Return>',   sub {$lglobal{spellpopup}->focus; spellreplace()});
                ...
                $lglobal{replacementlist}->bind('<Triple-Button-1>', sub{spellmisspelled_replace();spellreplace()});
                BindMouseWheel($lglobal{replacementlist});
                spelloptions() unless $globalspellpath;         # Check to see if we know where Aspell is
...
sub spelloptions{
                ...
                my $spellpathentry = $spellop->add('Entry', -width => 60, -background => 'white')->pack;
                my $spellpathbrowse = $spellop->add('Button', -text => 'Browse', -width => 12,
                        -command => sub{
                ...
                $dictlist = $spellop->add('ScrlListbox',
                                                -scrollbars => 'oe',
                                                -selectmode => 'browse',
                                                -background => 'white',
                                                -height => 10,
                ...
                my $spelldictxt = $spellop->add('ROText', -width => 40, -height => 1, -background => 'white')->pack;
                ...
                $spopframe->Radiobutton(-selectcolor => $lglobal{checkcolor},-text => 'Ultra Fast',-variable => \$globalaspellmode,-value => 'ultra')->grid(-row => 0, -sticky => 'w');
                $spopframe->Radiobutton(-selectcolor => $lglobal{checkcolor},-text => 'Fast', -variable => \$globalaspellmode,-value => 'fast')->grid(-row => 1, -sticky => 'w');
                $spopframe->Radiobutton(-selectcolor => $lglobal{checkcolor},-text => 'Normal', -variable => \$globalaspellmode,-value => 'normal')->grid(-row => 2, -sticky => 'w');
                $spopframe->Radiobutton(-selectcolor => $lglobal{checkcolor},-text => 'Bad Spellers', -variable => \$globalaspellmode,-value => 'bad-spellers')->grid(-row => 3, -sticky => 'w');
                $spellop->Show;
}
...
sub spellcheckfirst {                                                   # Initialize spellchecker
        ...
        spellget_misspellings();                                        # get list of mispelled words in selection (or file if nothing selected)
        ...
        $lglobal{matchindex} = $textwindow->search(-forwards, -count => \$lglobal{matchlength}, $term, $lglobal{spellindexstart},'end');        # search for the mispelled word in the text
        ...
        if (scalar($lglobal{seen})){
                $lglobal{misspelledlabel}->configure(-text => "Not in Dictionary:  -  $lglobal{seen}->{$term} in text.");
        }
        $lglobal{nextmiss} = 0;
}
...
sub getprojectdic{
        ...
        if ($lglobal{projectdictname} eq $lglobal{global_filename}){$lglobal{projectdictname} .= '.dic'}
}
...
sub spellchecknext {
        ...
        unless ($nobell){$textwindow->bell if ($lglobal{nextmiss} >= (scalar(@{$lglobal{misspelledlist}})))};
        $lglobal{suggestionlabel}->configure(-text => 'Suggestions:');
        return if $lglobal{nextmiss} >= (scalar(@{$lglobal{misspelledlist}}));  # no more mispelled words, bail
        $lglobal{lastmatchindex} = $textwindow->index('spellindex');
        #print $lglobal{misspelledlist}[$lglobal{nextmiss}]." | $lglobal{lastmatchindex}\n";
        if (($lglobal{misspelledlist}[$lglobal{nextmiss}]=~/^[\xC0-\xFF]/) || ($lglobal{misspelledlist}[$lglobal{nextmiss}]=~/[\xC0-\xFF]$/)){ # crappy workaround for accented character bug
                $lglobal{matchindex} = ($textwindow->search (-forwards, -count => \$lglobal{matchlength},$lglobal{misspelledlist}[$lglobal{nextmiss}],$lglobal{lastmatchindex},'end'));
        }else{
                $lglobal{matchindex} = ($textwindow->search (-forwards, -regexp, -count => \$lglobal{matchlength},'(?<!\p{Alpha})'.$lglobal{misspelledlist}[$lglobal{nextmiss}].'(?!\p{Alnum})',$lglobal{lastmatchindex},'end'));
        }
        unless ($lglobal{matchindex}){$lglobal{matchindex} = ($textwindow->search (-forwards, -exact, -count => \$lglobal{matchlength},$lglobal{misspelledlist}[$lglobal{nextmiss}],$lglobal{lastmatchindex},'end'))};
        $lglobal{spreplaceentry}->delete('0','end');                                            # remove last replacement word
        $lglobal{misspelledentry}->insert('end',$lglobal{misspelledlist}[$lglobal{nextmiss}]);  #put the misspelled word in the spellcheck text box
        spelladdtexttags() if $lglobal{matchindex};                                             # highlight the word in the text
        $lglobal{lastmatchindex} = spelladjust_index($lglobal{matchindex},$lglobal{misspelledlist}[$lglobal{nextmiss}]) if $lglobal{matchindex};        #get the index of the end of the match
        ...
        update_indicators();                                                                    # update the status bar
        $lglobal{spellpopup}->configure(-title => 'Current Dictionary - '.($globalspelldictopt||'<default>')." | $#{$lglobal{misspelledlist}} words to check.");
        if (scalar($lglobal{seen})){
                $lglobal{misspelledlabel}->configure(-text => 'Not in Dictionary:  -  '.($lglobal{seen}->{$lglobal{misspelledlist}[$lglobal{nextmiss}]}||'0').' in text.');
        }
        return 1;
}

sub spellgettextselection {
        return $textwindow->get ($lglobal{matchindex},"$lglobal{matchindex}+$lglobal{matchlength}c");   # get the misspelled word as it appears in the text (may be checking case insensitive)
}
...
sub spellreplace {
        ...
        $textwindow->replacewith($lglobal{matchindex},"$lglobal{matchindex}+$lglobal{matchlength}c",$replacement);
        $lglobal{lastmatchindex} = spelladjust_index(($textwindow->index($lglobal{matchindex})),$replacement);  #adjust the index to the end of the replaced word
        print OUT '$$ra '."$misspelled, $replacement\n";
        shift @{$lglobal{misspelledlist}};
        spellchecknext();                                                       # and check the next word
}

sub spellreplaceall { # replace all instances of a word with another, pretty straightforward
        $top->Busy;
        viewpagenums() if ($lglobal{seepagenums});
        my $lastindex = '1.0';
        my $misspelled = $lglobal{misspelledentry}->get;
...
sub spellmisspelled_replace {   # replace the replacement word with one from the guess list
        viewpagenums() if ($lglobal{seepagenums});
...
sub aspellstart{ # start aspell in interactive mode, repipe stdin and stdout to file handles
        aspellstop();
        my @cmd = ($lglobal{spellexename}, '-a', '-S', '--sug-mode', $globalaspellmode);
...
sub aspellstop{ # many thanks to Gregory Margo for some excellent patches for my lousy process handling
        if ($lglobal{spellpid}) {
                close IN;
                close OUT;
...
sub spellguesses {                                      #feed aspell a word to get a list of guess
        ...
        @{$lglobal{guesslist}} = (split /, /, $list);   # split the words into an array
...
sub spellcheckrange{ # only spell check selected text or whole file if nothing selected
        viewpagenums() if ($lglobal{seepagenums});
        my @ranges = $textwindow->tagRanges('sel');
...
sub spellget_misspellings {                                     # get list of misspelled words
        ...
        print SAVE $section;
        close SAVE;
        my $spellopt = get_spellchecker_version() lt "0.6" ? "-l " : "list --encoding=utf-8 ";
        $spellopt .= "-d $globalspelldictopt" if $globalspelldictopt;
        @templist = `$lglobal{spellexename} $spellopt < "checkfil.chk"`;        # feed the text to aspell, get an array of misspelled words out
        ...
        if ($#{$lglobal{misspelledlist}} > 0){
                ...
                $lglobal{spellpopup}->configure(-title => 'Current Dictionary - '.($globalspelldictopt||'<default>').' | No Misspelled Words Found.');
        }
...
sub spellignoreall{                                                     # remove ignored words from checklist
        ...
        foreach $next(@ignorelist){                                     # then put all of the words you are NOT ignoring back into the mispellings list
                push @{$lglobal{misspelledlist}}, $next if ($next ne $word);    # inefficient but easy, and the overhead isn't THAT bad...
        };
        spellmyaddword($word);
}
...
sub spelladdtexttags {                                                  # add highlighting to selected word
        ...
        $textwindow->tagAdd('highlight',$lglobal{matchindex},"$lglobal{matchindex}+$lglobal{matchlength} chars");
        $textwindow->yview('end');
...
sub searchsize{ # Pop up a window where you can adjust the search history size
        if ($lglobal{hssizepop}){
                ...
                my $frame = $lglobal{hssizepop}->Frame->pack(-fill => 'x', -padx => 5, -pady => 5);
                ...
                my $frame2 = $lglobal{hssizepop}->Frame->pack(-fill => 'x', -padx => 5, -pady => 5);
                $frame2->Button(
                        -text => 'Ok',
                        -width => 10,
                        -command => sub{
                ...
                $lglobal{hssizepop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{hssizepop}->destroy; undef $lglobal{hssizepop}});
...
sub searchpopup{
        viewpagenums() if ($lglobal{seepagenums});
        push @operations, (localtime().' - Search & Replace') unless $lglobal{doscannos};
        push @operations, (localtime().' - Stealth Scannos') if $lglobal{doscannos};
        oppopupdate() if $lglobal{oppop};
        my $aacheck;
        my $searchterm = '';
        my @ranges = $textwindow->tagRanges('sel');
        ...
        if (defined($lglobal{search})){
                ...
                my $sf1 = $lglobal{search}->Frame->pack(-side => 'top', -anchor => 'n');
                my $searchlabel = $sf1->Label(
                        -text => 'Search Text',
                ...
                my $sf11 = $lglobal{search}->Frame->pack(-side => 'top', -anchor => 'w', -padx => 3, -expand => 'y', -fill => 'x');

                $sf11->Button(
                        -activebackground => $activecolor,
                        -command => sub {$textwindow->undo;$textwindow->tagRemove('highlight','1.0','end');},
                        -text => 'Undo',
                        -width => 6
                )->pack(-side => 'right', -anchor => 'w');
                $lglobal{searchbutton} = $sf11->Button(
                        -activebackground => $activecolor,
                        -command => sub {add_search_history($lglobal{searchentry}, \@search_history); searchtext('');},
                        -text => 'Search',
                        -width => 6
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'w');

                $lglobal{searchentry} = $sf11->Text(
                        -background => 'white',
                        -width => 60,
                        -height => 1,
                )->pack(-side => 'right', -anchor => 'w', -expand => 'y', -fill => 'x');

                $sf11->Button(
                        -activebackground => $activecolor,
                        -command => sub {search_history($lglobal{searchentry}, \@search_history);},
                ...
                my $sf2 = $lglobal{search}->Frame->pack(-side => 'top', -anchor => 'w');
                ...
                my ($sf13, $sf14, $sf5);
                my $sf10 = $lglobal{search}->Frame->pack(-side => 'top', -anchor => 'n', -expand => '1', -fill => 'x');
                my $replacelabel = $sf10->Label(
                        -text => "Replacement Text\t\t",
                        )->grid(-row => 1, -column => 1);
                ...
                $sf10->Radiobutton(
                        ...
                        -command => sub{for ($sf13, $sf14){
                                                if (defined $sf5){
                                                        $_->pack(-before => $sf5, -side => 'top', -anchor => 'w', -padx => 3, -expand => 'y', -fill => 'x');
                                                }else{
                                                        $_->pack(-side => 'top', -anchor => 'w', -padx => 3, -expand => 'y', -fill => 'x');
                                                }
                                        }
                                },
                )->grid(-row => 1, -column => 4);
                my $sf12 = $lglobal{search}->Frame->pack(-side => 'top', -anchor => 'w', -padx => 3, -expand => 'y', -fill => 'x');

                $sf12->Button(
                        -activebackground => $activecolor,
                        -command => sub {replaceall($lglobal{replaceentry}->get('1.0','1.end'))},
                        -text => 'Rpl All',
                        -width => 5
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');

                $sf12->Button(
                        -activebackground => $activecolor,
                        -command => sub {replace($lglobal{replaceentry}->get('1.0','1.end')); add_search_history($lglobal{searchentry}, \@search_history); searchtext('')},
                        -text => 'R & S',
                        -width => 5
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');
                $sf12->Button(
                        -activebackground => $activecolor,
                        -command => sub {replace($lglobal{replaceentry}->get('1.0','1.end')); add_search_history($lglobal{replaceentry}, \@replace_history);},
                        -text => 'Replace',
                        -width => 6
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');
                ...
                $lglobal{replaceentry} = $sf12->Text(
                        ...
                        -width => 60,
                        -height => 1,
                )->pack(-side => 'right', -anchor => 'w', -padx => 1, -expand => 'y', -fill => 'x');
                $sf12->Button(
                        -activebackground => $activecolor,
                        -command => sub {search_history($lglobal{replaceentry}, \@replace_history);},
                        -image => $lglobal{hist_img},
                        -width => 9,
                        -height => 15,
                )->pack(-side => 'right', -anchor => 'w');
                ...
                $sf13->Button(
                        -activebackground => $activecolor,
                        -command => sub {replaceall($lglobal{replaceentry1}->get('1.0','1.end'))},
                        -text => 'Rpl All',
                        -width => 5
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');
                ...
                $sf13->Button(
                        -activebackground => $activecolor,
                        -command => sub {replace($lglobal{replaceentry1}->get('1.0','1.end')); searchtext('')},
                        -text => 'R & S',
                        -width => 5
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');
                $sf13->Button(
                        -activebackground => $activecolor,
                        -command => sub {replace($lglobal{replaceentry1}->get('1.0','1.end')); add_search_history($lglobal{replaceentry1}, \@replace_history);},
                        -text => 'Replace',
                        -width => 6
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');
                ...
                $sf13->Button(
                        -activebackground => $activecolor,
                        -command => sub {search_history($lglobal{replaceentry1}, \@replace_history);},
                        -image => $lglobal{hist_img},
                        -width => 9,
                        -height => 15,
                )->pack(-side => 'right', -anchor => 'w');
                ...
                $sf14->Button(
                        -activebackground => $activecolor,
                        -command => sub {replaceall($lglobal{replaceentry2}->get('1.0','1.end'))},
                        -text => 'Rpl All',
                        -width => 5
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');
                ...
                $sf14->Button(
                        -activebackground => $activecolor,
                        -command => sub {replace($lglobal{replaceentry2}->get('1.0','1.end')); searchtext('')},
                        -text => 'R & S',
                        -width => 5
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');
                $sf14->Button(
                        -activebackground => $activecolor,
                        -command => sub {replace($lglobal{replaceentry2}->get('1.0','1.end'));add_search_history($lglobal{replaceentry2}, \@replace_history);},
                        -text => 'Replace',
                        -width => 6
                )->pack(-side => 'right', -pady => 1, -padx => 2, -anchor => 'nw');
                ...
                $sf14->Button(
                        -activebackground => $activecolor,
                        -command => sub {search_history($lglobal{replaceentry2}, \@replace_history);},
                        -image => $lglobal{hist_img},
                        -width => 9,
                        -height =>      15,
                )->pack(-side => 'right', -anchor => 'w');
                ...
                if ($lglobal{doscannos}){
                        $sf5 = $lglobal{search}->Frame->pack(-side => 'top', -anchor => 'n');
                        my $nextbutton = $sf5->Button(
                                -activebackground => $activecolor,
                                -command => sub {$lglobal{scannosindex}++ unless ($lglobal{scannosindex} >= scalar(@{$lglobal{scannosarray}})); getnextscanno()},
                                -text => 'Next Stealtho',
                                -width => 15
                        )->pack(-side => 'left', -pady => 5, -padx => 2, -anchor => 'w');
                        my $lastbutton = $sf5->Button(
                                -activebackground => $activecolor,
                                -command => sub {$aacheck->deselect; $lglobal{scannosindex}-- unless ($lglobal{scannosindex} == 0); getnextscanno()},
                                -text => 'Prev Stealtho',
                                -width => 15
                        )->pack(-side => 'left', -pady => 5, -padx => 2, -anchor => 'w');
                        ...
                        my $sf6 = $lglobal{search}->Frame->pack(-side => 'top', -anchor => 'n');
                        $lglobal{regtracker} = $sf6->Label(-width => 15)->pack(-side => 'left', -pady => 5, -padx => 2, -anchor => 'w');
                        $aacheck = $sf6->Checkbutton(
                                -text => 'Auto Advance',
                                -variable => \$lglobal{regaa},
                        )->pack(-side => 'left', -pady => 5, -padx => 2, -anchor => 'w');
                }
                $lglobal{search}->protocol('WM_DELETE_WINDOW' => sub{
                ...
                $lglobal{search}->Tk::bind('<Return>' => sub{
                ...
                $lglobal{search}->Tk::bind('<Control-f>' => sub{
                                                        $lglobal{searchentry}->see('1.0');
                                                        $lglobal{searchentry}->delete('2.0','end');
                                                        $lglobal{replaceentry}->see('1.0');
                                                        $lglobal{replaceentry}->delete('2.0','end');
                ...
                $lglobal{search}->eventAdd('<<FindNexte>>' => '<Control-Key-G>','<Control-Key-g>');

                $lglobal{searchentry}->bind('<<FindNexte>>', sub {
                                $lglobal{searchentry}->delete('insert -1c') if ($lglobal{searchentry}->get('insert -1c') eq "\cG");
                ...
                $lglobal{searchentry}->bind('<FocusIn>', sub { $lglobal{hasfocus} = $lglobal{searchentry} });
                $lglobal{replaceentry}->bind('<FocusIn>', sub { $lglobal{hasfocus} = $lglobal{replaceentry} });
                $lglobal{replaceentry1}->bind('<FocusIn>', sub { $lglobal{hasfocus} = $lglobal{replaceentry1} });
                $lglobal{replaceentry2}->bind('<FocusIn>', sub { $lglobal{hasfocus} = $lglobal{replaceentry2} });

                $lglobal{search}->Tk::bind('<Control-Return>' => sub{
                                                        $lglobal{searchentry}->see('1.0');
                                                        $lglobal{searchentry}->delete('1.end');
                                                        $lglobal{searchentry}->delete('2.0','end');
                                                        $lglobal{replaceentry}->see('1.0');
                                                        $lglobal{replaceentry}->delete('1.end');
                                                        $lglobal{replaceentry}->delete('2.0','end');
                                                        replace($lglobal{replaceentry}->get('1.0','1.end')); searchtext();
                                                        $top->raise;
                                                        }
                );
                $lglobal{search}->Tk::bind('<Shift-Return>' => sub{
                                                        $lglobal{searchentry}->see('1.0');
                                                        $lglobal{searchentry}->delete('1.end');
                                                        $lglobal{searchentry}->delete('2.0','end');
                                                        $lglobal{replaceentry}->see('1.0');
                ...
                $lglobal{search}->Tk::bind('<Control-Shift-Return>' => sub{
                                                        $lglobal{searchentry}->see('1.0');
                                                        $lglobal{searchentry}->delete('1.end');
                                                        $lglobal{searchentry}->delete('2.0','end');
                                                        $lglobal{replaceentry}->see('1.0');
...
sub regedit{
        my $editor = $top->DialogBox(-title => 'Regex editor', -buttons => ['Save', 'Cancel']);
        my $regsearchlabel = $editor->add('Label', -text => 'Search Term')->pack;
        $lglobal{regsearch} = $editor->add('Text',
                -background => 'white',
                -width => 40,
                -height => 1,
        )->pack;
        ...
        $lglobal{reghinted} = $editor->add('Text',
                -background => 'white',
                -width => 40,
                -height => 8,
                -wrap => 'word',
        ...
        $buttonframe->Button(
                ...
                -text => '-->',
                -command => sub{
                        $lglobal{scannosindex}++ if $lglobal{scannosarray}[$lglobal{scannosindex}];
                        regload();
                        },
        )->pack(-side => 'left', -pady => 5, -padx => 2, -anchor => 'w');
        $buttonframe->Button(
        ...
        $lglobal{regsearch}->insert('end',($lglobal{searchentry}->get('1.0','1.end'))) if $lglobal{searchentry}->get('1.0','1.end');
        $lglobal{regsearch}->insert('end',($lglobal{replaceentry}->get('1.0','1.end'))) if $lglobal{replaceentry}->get('1.0','1.end');
        $lglobal{reghinted}->insert('end',($reghints{$lglobal{searchentry}->get('1.0','1.end')})) if $reghints{$lglobal{searchentry}->get('1.0','1.end')};
        ...
        if ($button =~ /save/i){
                ...
                print REG ");\n\n";
                print REG '# For a hint, use the regex expression EXACTLY as it appears in the %scannoslist hash'."\n";
                print REG '# but replace the replacement term (heh) with the hint text. Note: if a single quote'."\n";
                print REG '# appears anywhere in the hint text, you\'ll need to escape it with a backslash. I.E. isn\\\'t'."\n";
                print REG '# I could have made this more compact by converting the scannoslist hash into a two dimensional'."\n";
...
sub regload{
        ...
        $lglobal{regsearch}->insert('end',$scannoslist{$word}) if defined $word;
...
sub regadd{
        ...
        unless (defined $scannoslist{$st}){
                ...
                foreach ( sort (keys %scannoslist)) {push @{$lglobal{scannosarray}}, $_};
...
sub regdel{
        ...
        @{$lglobal{scannosarray}} = ();
        foreach $word ( sort (keys %scannoslist)) {push @{$lglobal{scannosarray}}, $word};
        regload();
}
...
sub reghint{
        ...
        if (defined($lglobal{hintpop})){
                ...
                my $frame = $lglobal{hintpop}->Frame->pack(-anchor => 'nw', -expand => 'yes', -fill => 'both');
                $lglobal{hintmessage} = $frame->ROText(
                        -width => 40,
                        -height => 6,
                        -background => 'white',
                        -wrap => 'word',
                )->pack(-anchor => 'nw', -expand => 'yes', -fill => 'both', -padx => 4, -pady => 4);
                $lglobal{hintmessage}->insert('end',$message);
        }
        $lglobal{hintpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{hintpop}->destroy; undef $lglobal{hintpop}});
...
sub loadscannos{
        ...
        $lglobal{scannosfilename} = $top->getOpenFile(-filetypes => $types, -title => 'Scannos list?', -initialdir => $scannospath);
        if ($lglobal{scannosfilename}){
                my ($name,$path,$extension) = fileparse($lglobal{scannosfilename},'\.[^\.]*$');
                $scannospath = $path;
                unless (my $return = do $lglobal{scannosfilename}) {            # load scannos list
...
sub getnextscanno{
        ...
        unless (searchtext()){
                if ($lglobal{regaa}){
                        while (1){last if ($lglobal{scannosindex}++ >= $#{$lglobal{scannosarray}}); findascanno(); last if searchtext();}
                };
        };
}
...
sub findascanno{
        ...
        $lglobal{regtracker}->configure(-text => ($lglobal{scannosindex}+1).'/'.scalar(@{$lglobal{scannosarray}}));
        $lglobal{hintmessage}->delete('1.0','end') if (defined($lglobal{hintpop}));
        return 0 unless $word;
        $lglobal{searchentry}->insert('end',$word);
        $lglobal{replaceentry}->insert('end',($scannoslist{$word}));
        $sopt[2] ? $textwindow->markSet('insert','end') : $textwindow->markSet('insert','1.0');
...
sub searchtext{
        ...
        $lglobal{lastsearchterm} = 'stupid variable needs to be initialized' unless length($lglobal{lastsearchterm});
        ...
        if ($start){$sopt[2] ? ($searchstartindex = $start) : ($searchendindex = "$start+1c")};
        {
        local($^W); # Turn off warnings temporarily since $searchterm is undefined on first search
        unless (length($searchterm)){$searchterm = $lglobal{searchentry}->get('1.0','1.end')};
        }
        ...
        unless ($searchterm eq $lglobal{lastsearchterm}){
                ($range_total == 0 ) ? ($searchendindex = '1.0') : ($searchendindex = $start);
                if ($sopt[2]){($range_total == 0) ? ($searchstartindex = 'end') : ($searchstartindex = $end)};
                $lglobal{lastsearchterm} = $searchterm unless (($searchterm =~ m/\\n/)&&($sopt[3]));
        ...
        if (($searchterm =~ m/\\n/)&&($sopt[3])){
                ...
                $exactsearch =~ s/([\{\}\[\]\(\)\^\$\.\|\*\+\?\\])/\\$1/g;      # escape metacharacters for whole word matching
                $searchterm = '(?<!\p{Alnum})'.$exactsearch.'(?!\p{Alnum})' if $sopt[0];
                my ($direction,$searchstart,$mode);
                if($sopt[2]){$searchstart = $searchstartindex}else{$searchstart = $searchendindex};
                if($sopt[2]){$direction = '-backwards'}else{$direction = '-forwards'};
                if($sopt[0] or $sopt[3]){$mode = '-regexp'}else{$mode = '-exact'};
                if($sopt[1]){$searchstartindex = $textwindow->search($mode, $direction, '-nocase', '-count' => \$length, '--', $searchterm, $searchstart, $end)
                }else{$searchstartindex = $textwindow->search($mode, $direction, '-count' => \$length, '--', $searchterm, $searchstart, $end)};
        ...
        if ($searchstartindex){
                ...
                $searchendindex = $textwindow->index("$searchstartindex +${length}c") if ($searchterm =~ m/\\n/);
                $searchendindex = $textwindow->index("$searchstartindex +1c") unless $length;
                $textwindow->markSet('insert',$searchstartindex) if $searchstartindex;  # position the cursor at the index
                $textwindow->tagAdd('highlight',$searchstartindex,$searchendindex)if $searchstartindex; # highlight the text
                $textwindow->yviewMoveto(1);
                $textwindow->see($searchstartindex)if ($searchendindex && $sopt[2]);                            # scroll text box, if necessary, to make found text visible
...
sub updatesearchlabels{
        if ($lglobal{seen}&&$lglobal{search}){
                ...
                if (($lglobal{seen}->{$searchterm1})&&($sopt[0])){
                        $lglobal{searchnumlabel}->configure(-text => "Found $lglobal{seen}->{$searchterm1} times.")
        ...
        if ($replaceterm =~ /\\C/){
                if ($lglobal{codewarn}){
                        my $dialog = $top->Dialog(
                                -text => "WARNING!! The replacement term will execute arbitrary perl code. ".
...
sub opstop{
        if (defined( $lglobal{stoppop})){
                ...
                $lglobal{stoppop}->protocol('WM_DELETE_WINDOW'=>sub{$lglobal{stoppop}->destroy; undef $lglobal{stoppop}});
...
sub replaceall{
        ...
        my @ranges = $textwindow->tagRanges('sel');
        if(@ranges){
                $lglobal{lastsearchterm} = $lglobal{replaceentry}->get('1.0','1.end');
...
sub nextblock{
        ...
        if ($mark eq 'default'){
                if ($direction eq 'forward'){
                        $searchstartindex = $textwindow->search('-exact','--', '/*', $searchstartindex, 'end') if $searchstartindex;
                }elsif($direction eq 'reverse'){
                        $searchstartindex = $textwindow->search('-backwards', '-exact','--', '/*', $searchstartindex, '1.0') if $searchstartindex;
                }
        }elsif ($mark eq 'indent'){
                if ($direction eq 'forward'){
                        $searchstartindex = $textwindow->search('-regexp','--', '^\S', $searchstartindex, 'end') if $searchstartindex;
                        $searchstartindex = $textwindow->search('-regexp','--', '^\s', $searchstartindex, 'end') if $searchstartindex;
                }elsif($direction eq 'reverse'){
                        $searchstartindex = $textwindow->search('-backwards', '-regexp','--', '^\S', $searchstartindex, '1.0') if $searchstartindex;
                        $searchstartindex = $textwindow->search('-backwards', '-regexp','--', '^\s', $searchstartindex, '1.0') if $searchstartindex;
                }
        }elsif ($mark eq 'stet'){
                if ($direction eq 'forward'){
                ...
                if ($direction eq 'forward'){
                        $searchstartindex = $textwindow->search('-regexp','--', '\/[pP]', $searchstartindex, 'end') if $searchstartindex;
                }elsif($direction eq 'reverse'){
                        $searchstartindex = $textwindow->search('-backwards', '-regexp','--', '\/[pP]', $searchstartindex, '1.0') if $searchstartindex;
                }
        }
...
sub brackets{
        ...
        if (defined($lglobal{brkpop})){
                ...
                my $brnextbt = $frame2->Button(
                        ...
                        -command => sub{
                                ...
                                $textwindow->bell unless ($lglobal{brbrackets}[1]||$nobell);
                                return unless $lglobal{brbrackets}[1];
                                brnext()},
                        -width => 10,
                )->grid(-row => 2, -column => 2, -pady => 5);
        }
        $lglobal{brkpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{brkpop}->destroy; undef $lglobal{brkpop}; $textwindow->tagRemove('highlight','1.0','end')});
        ...
        sub brsearch{
                ...
                while ($lglobal{brindex}) {
                        $lglobal{brindex} = $textwindow->search('-regexp', '-count' => \$brlength, '--', $lglobal{brsel}, $lglobal{brindex}, 'end');
                        last unless $lglobal{brindex};
                        $lglobal{brbrackets}[$brcount]=$textwindow->get($lglobal{brindex},$lglobal{brindex}.'+'.$brlength.'c');
        ...
        sub brnext{
                ...
                $textwindow->tagRemove('highlight','1.0','end');
                while (1){
                        last unless ((($lglobal{brbrackets}[0] =~ /[\[\(\{\<«]/)&&($lglobal{brbrackets}[1] =~ /[\]\)\}\>»]/))||
                        (($lglobal{brbrackets}[0] =~ /^\x7f*\/\*/)&&($lglobal{brbrackets}[1] =~ /^\x7f*\*\//))||
                        (($lglobal{brbrackets}[0] =~ /^\x7f*\/\$/)&&($lglobal{brbrackets}[1] =~ /^\x7f*\$\//))||
                        (($lglobal{brbrackets}[0] =~ /^\x7f*\/[Pp]/)&&($lglobal{brbrackets}[1] =~ /^\x7f*[Pp]\//))||
                        (($lglobal{brbrackets}[0] =~ /^\x7f*\/\#/)&&($lglobal{brbrackets}[1] =~ /^\x7f*\#\//)));
                ...
                if (($lglobal{brbrackets}[2])&&($lglobal{brbrackets}[3])){
                        if (($lglobal{brbrackets}[0] eq $lglobal{brbrackets}[1])&&($lglobal{brbrackets}[2] eq $lglobal{brbrackets}[3])){
                                shift @{$lglobal{brbrackets}};
                                shift @{$lglobal{brbrackets}};
                                shift @{$lglobal{brindicies}};
                                shift @{$lglobal{brindicies}};
                ...
                $textwindow->markSet('insert',$lglobal{brindicies}[0]) if $lglobal{brindicies}[0];
                $textwindow->see($lglobal{brindicies}[0]) if $lglobal{brindicies}[0];
                $textwindow->tagAdd('highlight',$lglobal{brindicies}[0],$lglobal{brindicies}[0].'+'.(length($lglobal{brbrackets}[0])).'c')if $lglobal{brindicies}[0];
                $textwindow->tagAdd('highlight',$lglobal{brindicies}[1],$lglobal{brindicies}[1].'+'.(length($lglobal{brbrackets}[1])).'c')if $lglobal{brindicies}[1];
                $textwindow->focus;
                }
        }
}
...
sub orphans{
        ...
        $open = '<'.$br.'>';
        $open = '<'.$br.'>|<'.$br.' [^>]*>' if (($br eq 'p')||($br eq 'span'));
        $close = '<\/'.$br.'>';
        my $end = $textwindow->index('end');
        $thisindex = '1.0';
        my ($lengtho,$lengthc);
        my $opindex = $textwindow->search('-regexp', '-count' => \$lengtho,'--',$open, $thisindex, 'end');
        push @op,$opindex;
        my $clindex = $textwindow->search('-regexp', '-count' => \$lengthc,'--',$close, $thisindex, 'end');
        return unless ($clindex||$opindex);
        push @op, ($clindex || $end);
        while ($opindex){
                $opindex = $textwindow->search('-regexp', '-count' => \$lengtho,'--',$open, $op[0].'+1c', 'end');
                if ($opindex){
                        push @op,$opindex
                }else{
                ...
                $clindex = $textwindow->search('-regexp', '-count' => \$lengthc,'--',$close, "$begin+1c", 'end');
                if ($clindex){
                        push @op,$clindex
                }else{
                        push @op,$textwindow->index('end')
                ...
                if (($textwindow->compare($op[0],'<',$op[1]))&&($textwindow->compare($op[1],'<',$op[2]))&&
                ($textwindow->compare($op[2],'<',$op[3]))&&($op[2] ne $end)&&($op[3] ne $end)){
                        $textwindow->update;
                        $textwindow->focus;
                        shift @op;
                        shift @op;
                        next;
                }elsif(($textwindow->compare($op[0],'<',$op[1]))&&($textwindow->compare($op[1],'>',$op[2]))){
                        ...
                        $textwindow->focus;
                        return 1;
                }elsif(($textwindow->compare($op[0],'<',$op[1]))&&($textwindow->compare($op[2],'>',$op[3]))){
                        ...
                        $textwindow->focus;
                        return 1;
                }elsif(($textwindow->compare($op[0],'>',$op[1]))&&($op[0] ne $end)){
                        ...
                        if (($op[3] eq $end)&&($textwindow->compare($op[2],'>',$op[0]))){
                                $textwindow->markSet('insert',$op[2])if $op[2];
                                $textwindow->see($op[2]) if $op[2];
                                $textwindow->tagAdd('highlight',$op[2],$op[2].'+'.$lengthc.'c');
                        }
                        if (($op[2] eq $end)&&($textwindow->compare($op[3],'>',$op[1]))){
                                $textwindow->markSet('insert',$op[3])if $op[3];
                                $textwindow->see($op[3]) if $op[3];
                                $textwindow->tagAdd('highlight',$op[3],$op[3].'+'.$lengthc.'c');
                        }
                ...
                while (@ranges){
                        ...
                        if ($thisblockstart == $thisblockend){
                                ...
                                if ($indent eq 'in'){
                                        ...
                                        $textwindow->insert($start,$char)unless ($textwindow->get($start,"$start lineend") =~ /^$/);
                                        $end = "$end+1c" unless ($textwindow->get($end,"$end lineend") =~ /^$/);
                                ...
                                while ($index <= $thisblockend){
                                        if ($indent eq 'in'){
                                                $textwindow->insert($index,' ')unless ($textwindow->get($index,"$index lineend") =~ /^$/);
                                        }elsif($indent eq 'out'){
                        ...
                        if($indent eq 'up'){
                                my $temp = $end, $end = $start; $start = $temp;
                                if ($textwindow->compare("$start linestart", '==', '1.0')){
                                        ...
                                        $textwindow->replacewith("$start-1l", "$end-1l",($textwindow->get($start, $end)));
...
sub selectrewrap{
        ...
        if($range_total == 0){
                ...
                while(1){
                        ...
                        if($thisblockend){$thisblockend = $textwindow->index($thisblockend.' lineend')}else{$thisblockend = $end};      #or end of text if end of selection
                        my $selection = $textwindow->get($thisblockstart,$thisblockend)if $thisblockend;                #get the paragraph of text
                        unless ($selection){
                        ...
                        last if (($thisblockend eq $lastend)||($textwindow->compare($thisblockend,'<',$lastend)));      #quit if the search isn't advancing
                        ...
                        if ($selection =~ /^\x7f*\/\#/){
                                ...
                                if ($selection =~ /^\x7f*\/#\[(\d+)/){                                  #check for block rewrapping with parameter markup
                                        if ($1){$leftmargin = $1+1};
                        ...
                        if ($selection =~ /^\x7f*\/[\*Ll]/){$inblock = 1; $enableindent = 1}                            #check for no rewrap markup
                        if ($selection =~ /^\x7f*\/\*\[(\d+)/){$indent = $1}
                        if ($selection =~ /^\x7f*\/[pP]/){$inblock = 1; $enableindent = 1; $poem = 1; $indent = 4}
                        ...
                        unless($selection =~ /^\x7f*\s*?(\*\s*){4}\*/){                         #skip rewrap if paragraph is a thought break
                                if ($inblock){
                                        if ($enableindent){
                                                ...
                                                unless ($offset){
                                                        ...
                                                        for $line($sr+1..$er-1){
                                                                ...
                                                                if ($textline){
                                                                        $textwindow->search('-regexp','-count' => \$spaces, '--', '^\s+', "$line.0", "$line.end");
                                                                        unless ($spaces){$spaces = 0};
                                                                        if ($spaces < $offset){$offset = $spaces};
                                                                        $spaces = 0;
                                                                }
                                                        }
                                                ...
                                                for $line($sr..$er-1){
                                                        $textline = $textwindow->get("$line.0", "$line.end");
                                                        next if (($textline =~ /^\x7f*\/[pP\*Ll]/)||($textline =~ /^\x7f*[pP\*Ll]\//));
                                                        if ($enableindent){
                                                                $textwindow->insert("$line.0", (' ' x $indent)) if ($indent > 0);
                                                                if ($indent < 0) {
                                                                        if ($textwindow->get("$line.0", "$line.@{[abs $indent]}") =~ /\S/){
                                                                                while ($textwindow->get("$line.0") eq ' '){$textwindow->delete("$line.0")}
                                                                        }else{
                                        ...
                                        $selection =~ s/\)/\x98/g;
                                        if ($blockwrap){
                                                $rewrapped = wrapper($leftmargin,$firstmargin,$rightmargin,$selection);
                                        }else{                                                  #rewrap the paragraph
                                                $rewrapped = wrapper($lmargin,$lmargin,$rmargin,$selection);
                                        }
                        ...
                        if ($selection =~ /^\x7f*[XxFf\$]\//m){$inblock = 0; $indent = 0; $offset = 0; $enableindent = 0; $poem = 0};
                        ...
                        $blockwrap = 0 if $operationinterrupt;          #reset blockwrap if rewrap routine is interrupted
                        last if $operationinterrupt;                            #then quit
                        last if ($thisblockstart eq $end);                      #quit if next paragrapn starts at end of selection
                        update_indicators();                                            # update line and page numbers
                }
        if ($lglobal{stoppop}){$lglobal{stoppop}->destroy; undef $lglobal{stoppop}};    #destroy interrupt popup
        $operationinterrupt = 0;
        $textwindow->focus;
        $textwindow->update;
...
sub wrapper{
        ...
        while (@words){
                ...
                if ($word =~ /\/#/){
                        ...
                        if ($word =~ /^\x7f*\/#\x8A(\d+)/){             #check for block rewrapping with parameter markup
                                if (length $1){
                                        $leftmargin = $1;
                                        $firstmargin = $leftmargin;
                                };
        ...
        if ($paragraph =~ /-[#\*]\//){  # Trap bug when there is a hyphen at the end of a block
                $paragraph =~ s/\n(\S+)-([#\*]\/)/ $1-\n$2/;
        }
        return($paragraph);
}
...
sub setmargins{
        ...
        $getmargins->Show;
        if (($blockrmargin eq '')||($blocklmargin eq '')||($rmargin eq '')||($lmargin eq '')){
                $top->messageBox(
                        -icon => 'error',
        ...
        if (($blockrmargin =~ /[\D\.]/)||($blocklmargin =~ /[\D\.]/)||($rmargin =~ /[\D\.]/)||($lmargin =~ /[\D\.]/)){
                $top->messageBox(
                        -icon => 'error',
                        -message => 'The margins must be a positive integer.',
                        -title => 'Incorrect margin ',
...
sub asciipopup{
        ...
        if (defined($lglobal{asciipop})){
                ...
                my $f = $lglobal{asciipop}->Frame->pack(-side => 'top', -anchor => 'n');
                $f->Label(
                -text => 'ASCII Drawing Characters',
                )->pack(-side => 'top', -pady => 2, -padx => 2, -anchor => 'n');
                my $f5 = $lglobal{asciipop}->Frame->pack(-side => 'top', -anchor => 'n');
                my ($row, $col);
                for (0..8) {
                        next if $_ == 4;
                        $row = int $_ / 3;
                ...
                my $f1 = $lglobal{asciipop}->Frame->pack(-side => 'top', -anchor => 'n');
                ...
                $lglobal{asciipop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{asciipop}->destroy; undef $lglobal{asciipop}});
...
sub asciibox{
        ...
        if($range_total == 0){
                ...
                $lmargin = $saveleft;
                $rmargin = $saveright;
                $textwindow->insert('asciistart',${$lglobal{ascii}}[0].(${$lglobal{ascii}}[1] x ($lglobal{asciiwidth}-2)).${$lglobal{ascii}}[2]."\n");
                $textwindow->insert('asciiend',"\n".${$lglobal{ascii}}[6].(${$lglobal{ascii}}[7] x ($lglobal{asciiwidth}-2)).${$lglobal{ascii}}[8]."\n");
                ...
                $line = ${$lglobal{ascii}}[3].(' ' x $lspaces).$line.(' ' x $rspaces).${$lglobal{ascii}}[5];
...
sub alignpopup{
        if (defined($lglobal{alignpop})){
                ...
                $f->Label(
                -text => 'String to align on (first occurence)',
                )->pack(-side => 'top', -pady => 5, -padx => 2, -anchor => 'n');
                ...
                $lglobal{alignpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{alignpop}->destroy; undef $lglobal{alignpop}});
...
sub aligntext{
        ...
        if($range_total == 0){
                ...
                ($er,$ec) = split /\./, $end;
                for $linenum($sr..$er-1){
                        $indexpos[$linenum] = $textwindow->search('--', $lglobal{alignstring}, "$linenum.0 -1c", "$linenum.end");
                        if ($indexpos[$linenum]){($r,$c) = split /\./, $indexpos[$linenum]}else{$c = -1}
...
sub flood{
        if (defined($lglobal{floodpop})){
                ...
                $f->Label(
                -text => "Flood fill string.\n(Blank will default to spaces.)\nHotkey Control+w",
                )->pack(-side => 'top', -pady => 5, -padx => 2, -anchor => 'n');
                my $f1 = $lglobal{floodpop}->Frame->pack(-side => 'top', -anchor => 'n',-expand => 'y',-fill => 'x');
                my $floodch = $f1->Entry(
                        -background => 'white',
                        -font => $lglobal{font},
                        -relief => 'sunken',
                        -textvariable => \$lglobal{ffchar},
                )->pack(-side => 'left', -pady => 5, -padx => 2, -anchor => 'w',-expand => 'y',-fill => 'x');
                ...
                $lglobal{floodpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{floodpop}->destroy; undef $lglobal{floodpop}});
...
sub floodfill{
        ...
        while (@ranges){
                ...
                my $temp = substr($lglobal{ffchar} x (((length $selection)/(length $lglobal{ffchar}))+1),0,(length $selection));
                chomp $selection;
                my @temparray = split (/\n/, $selection);
                my $replacement;
                for (@temparray){
...
sub surround{
        if (defined($lglobal{surpop})){
                ...
                $f->Label(
                -text => "Surround the selection with?\n\\n will be replaced with a newline.",
                )->pack(-side => 'top', -pady => 5, -padx => 2, -anchor => 'n');
                ...
                $lglobal{surpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{surpop}->destroy; undef $lglobal{surpop}});
...
sub markpopup{
        ...
        if (defined($lglobal{markpop})){
                ...
                my $tableformat;
                my $f0 = $lglobal{markpop}->Frame->pack(-side => 'top', -anchor => 'n');
                $f0->Button(
                        -activebackground => $activecolor,
                ...
                my $pagecomments = $f0->Checkbutton(
                        ...
                        -anchor => 'w',
                        )->grid(-row => 2, -column => 1, -padx => 1, -pady => 2, -sticky => 'w');
                my $pageanchors = $f0->Checkbutton(
                        -variable => \$lglobal{pageanch},
                        -selectcolor => $lglobal{checkcolor},
                        -text => 'Insert Anchors at Pg #s',
                        -anchor => 'w',
                        )->grid(-row => 2, -column => 3, -padx => 1, -pady => 2, -sticky => 'w');
                $pageanchors->select;
                my $fractions = $f0->Checkbutton(
                        ...
                        -anchor => 'w',
                        )->grid(-row => 3, -column => 1, -padx => 1, -pady => 2, -sticky => 'w');
                my $utfconvert = $f0->Checkbutton(
                        -variable => \$lglobal{leave_utf},
                        -selectcolor => $lglobal{checkcolor},
                        -text => 'Keep UTF-8 Chars',
                        -anchor => 'w',
                        )->grid(-row => 3, -column => 2, -padx => 1, -pady => 2, -sticky => 'w');
                $blockmarkup = $f0->Checkbutton(
                        ...
                        -text => 'CSS blockquote',
                        -anchor => 'w',
                        )->grid(-row => 3, -column => 3, -padx => 1, -pady => 2, -sticky => 'w');
                my $f1 = $lglobal{markpop}->Frame->pack(-side => 'top', -anchor => 'n');
                my ($inc, $row, $col) = (0,0,0);
                for (qw/i b u center h1 h2 h3 h4 h5 h6 p hr br big small ol ul li sup sub table tr td blockquote code/){
                        $col = $inc % 5;
                        $row = int $inc / 5;
                        $f1->Button(
                                -activebackground => $activecolor,
                ...
                my %hbuttons = ('anchor','Named anchor','img','Image','elink','External Link','ilink','Internal Link');
                ($row, $col) = (0,0);
                for (keys %hbuttons){
                        $f2->Button(
                                -activebackground => $activecolor,
                ...
                $f3->Button(
                        -activebackground => $activecolor,
                        -command => sub {
                                                        for my $orphan ('b','i','center','u','sub','sup','sc','h1','h2','h3','h4','h5','h6','p','span'){
                ...
                $f4->Button(
                        -activebackground => $activecolor,
                        -command => sub { autotable($tableformat->get); $textwindow->focus},
                        -text => 'Auto Table',
                        -width => 16
                ...
                $f5->Button(
                        -activebackground => $activecolor,
                        -command => sub {markup('div', $diventry->get); $textwindow->focus},
                        -text => 'div',
                        -width => 8
                        )->grid(-row => 1, -column => 2, -padx => 2, -pady => 2);
                my $f6 = $lglobal{markpop}->Frame->pack(-side => 'top', -anchor => 'n');
                ...
                $f6->Button(
                                -activebackground => $activecolor,
                                -command => sub {markup('span', $spanentry->get); $textwindow->focus},
                                -text => 'span',
                                -width => 8
                        )->grid(-row => 1, -column => 2, -padx => 2, -pady => 2);
                ...
                $f7->Button(
                        -activebackground => $activecolor,
                        -command => sub {open my $infile, '<', 'header.txt' or warn "Could not open header file. $!\n";
                                                        my $headertext;
                                                        while (<$infile>){
                                                                $_ =~ s/\cM\cJ|\cM|\cJ/\n/g;
                                                                $headertext .= $_;
                ...
                $f8->Button(
                        -activebackground => $activecolor,
                        -command => sub{tidyrun('-f tidyerr.err -o null'); unlink 'null' if (-e 'null')},
                        -text => 'HTML Tidy',
                        -width => 16
                )->grid(-row => 1, -column => 2, -padx => 1, -pady => 2);
                $diventry->insert('end',' style="margin-left: 2em;"');
                $spanentry->insert('end',' style="margin-left: 2em;"');
                $lglobal{markpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{markpop}->destroy; undef $lglobal{markpop}});
...
sub poetryhtml{
        ...
        if($range_total == 0){
                ...
                if (length $selection){$selection = "<span$class>".$selection.'<br /></span>'}else{$selection = ''}
                ...
                $textwindow->insert("$lsr.0", "<div class=\"poem\"><div class=\"stanza\">\n");
        }
}
...
sub autotable{
        ...
        my @ranges = $textwindow->tagRanges('sel');
        unless (@ranges){push @ranges, $textwindow->index('insert'); push @ranges, $textwindow->index('insert')};
        my $range_total = @ranges;
        if($range_total == 0){
                return;
        }else{
        ...
        if($range_total == 0){
                ...
                if ($lglobal{list_multiline}){
                        ...
                        while ($step <= $ler){
                                ...
                                unless ($paragraph){unless ($selection =~ /<li>/){$selection = '<li>'.$selection.'</li>'}};
                                $selection =~ s/<li><\/li>//;
...
sub markup{
        ...
        if($range_total == 0){
                ...
                if($mark eq 'del'){
                        ...
                        while ($step <= $ler){
                                ...
                                $edited++ if ($selection =~ s/<span.*?margin-left: (\d+\.?\d?)em.*?>/' ' x ($1 *2)/e);
                        ...
                        if ($lglobal{elinkpop}){
                                ...
                                my $linkf1 = $lglobal{elinkpop}->Frame->pack(-side => 'top', -anchor => 'n');
                                my $linklabel = $linkf1->Label(-text => 'Link name')->pack;
                                $lglobal{linkentry} = $linkf1->Entry(-width => 60, -background => 'white')->pack;
                                my $linkf2 = $lglobal{elinkpop}->Frame->pack(-side => 'top', -anchor => 'n');
                                my $extbrowse = $linkf2->Button(
                                        -activebackground => $activecolor,
                                        -text => 'Browse', -width => 16,
                                        -command => sub{
                                ...
                                $lglobal{elinkpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{elinkpop}->destroy; undef $lglobal{elinkpop}});
                        ...
                        my ($anchorname,$anchorstartindex, $anchorendindex, $length, $srow, $scol, $string,$link,$match,$match2);
                        $length = 0;
                        @intanchors = ();
                        my %inthash = ();
                        $anchorstartindex = $anchorendindex = '1.0';
                        while ($anchorstartindex = $textwindow->search('-regexp', '--','<a (name|id)=[\'"].+?[\'"]',$anchorendindex, 'end')){
                                $anchorendindex = $textwindow->search('-regexp', '--','>',$anchorstartindex, 'end');
                        ...
                        if ($lglobal{linkpop}){
                                ...
                                $tframe->Checkbutton(
                                        ...
                                        -command => sub{$linklistbox->delete('0','end'); linkpopulate($linklistbox,\@intanchors);},
                                )->pack(-side => 'left', -pady => 2, -padx => 2, -anchor => 'n');
                                $tframe->Checkbutton(
                                ...
                                $linklistbox = $pframe->Scrolled('Listbox',
                                        -scrollbars => 'se',
                                        -background => 'white',
                                        -selectmode => 'single',
                                        -activestyle => 'none',
                                )->pack(-side => 'top', -anchor => 'nw', -fill => 'both', -expand => 'both', -padx => 2, -pady => 2);
                                drag($linklistbox);
                                $lglobal{linkpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{linkpop}->destroy; undef $lglobal{linkpop}});
                                $lglobal{linkpop}->Icon(-image => $icon);
                                BindMouseWheel($linklistbox);
                                $linklistbox->eventAdd('<<trans>>' => '<Double-Button-1>');
                                $linklistbox->bind('<<trans>>', sub {
                                ...
                                for (sort (@intanchors)){
                                        last unless $tempvar;
                                        next if ((($_ =~ /#Footnote/)||($_ =~ /#FNanchor/))&&$lglobal{fnlinks});
                                ...
                                if ($entrarray[1] && ($entrarray[1] ne '@')){
                                        $entrarray[0] = '@' if ($entrarray[0] =~ /^to$|^a$|^the$|^and$/);
                                        $entrarray[1] = '@' if ($entrarray[1] =~ /^to$|^a$|^the$|^and$/);
                                        $entrarray[2] = '@' if ($entrarray[2] =~ /^to$|^a$|^the$|^and$/);
                                }
                                for (sort (@intanchors)){
                                        next if ((($_ =~ /#Footnote/)||($_ =~ /#FNanchor/))&&$lglobal{fnlinks});
                                        next if (($_ =~ /#Page_\d+/)&&$lglobal{pglinks});
                                        next unless (lc($_) =~ /\Q$entrarray[0]\E|\Q$entrarray[1]\E|\Q$entrarray[2]\E/);
                                        $linklistbox->insert('end',$_);
                                        $flag++;
                                }
                                $linklistbox->insert('end',"  ");
                        ...
                        $done = "<a name=\"".$linkname."\" id=\"".$linkname."\"></a>";
                        $textwindow->insert($thisblockstart,$done);
                }elsif($mark =~ /h\d/){
                        $selection = $textwindow->get($thisblockstart,$thisblockend);
                        if ($selection =~ s/<\/?p>//g){
        ...
        if($open != $close){
                $top->messageBox(
                        -icon => 'error',
                        -message => "Mismatching open and close markup removed.\nYou may have orphaned markup.",
...
sub htmlimage{
        ...
        if (defined($lglobal{htmlimpop})){
                ...
                $lglobal{htmlimpop}->geometry($lglobal{htmlgeom}) if $lglobal{htmlgeom};
                my $f1 = $lglobal{htmlimpop}->LabFrame(-label => 'File Name')->pack(-side => 'top', -anchor => 'n', -padx => 2);
                $lglobal{imgname} = $f1->Entry(
                        -width => 45,
                )->pack(-side => 'left');
                my $f3 = $lglobal{htmlimpop}->LabFrame(-label => 'Alt text')->pack(-side => 'top', -anchor => 'n');
                $lglobal{alttext} = $f3->Entry(
                        -width => 45,
                )->pack(-side => 'left');
                my $f4 = $lglobal{htmlimpop}->LabFrame(-label => 'Title text')->pack(-side => 'top', -anchor => 'n');
                $lglobal{titltext} = $f4->Entry(
                        -width => 45,
                )->pack(-side => 'left');
                my $f5 = $lglobal{htmlimpop}->LabFrame(-label => 'Geometry')->pack(-side => 'top', -anchor => 'n');
                ...
                $lglobal{htmlimggeom} = $f52->Label(-text => '')->pack(-side => 'left');
                my $f2 = $lglobal{htmlimpop}->LabFrame(-label => 'Alignment')->pack(-side => 'top', -anchor => 'n');
                $f2->Radiobutton(
                        -variable => \$alignment,
                        -text => 'Left',
                        -selectcolor => $lglobal{checkcolor},
                ...
                $f8->Button(
                        ...
                        -command => sub{
                                ...
                                if ($name) {
                                        my $sizexy = 'width="'.$lglobal{widthent}->get.'" height="'.$lglobal{heightent}->get.'"';
                                        ...
                                        if ($alignment eq 'center'){
                                                $textwindow->delete('thisblockstart','thisblockend');
                                                $textwindow->insert('thisblockstart',"<div class=\"figcenter\" style=\"width: ".$width."px;\">\n<img src=\"$name\" $sizexy alt=\"$alt\" title=\"$title\" />\n$selection</div>$preservep");
                                        }elsif($alignment eq 'left'){
                                                $textwindow->delete('thisblockstart','thisblockend');
                                                $textwindow->insert('thisblockstart',"<div class=\"figleft\" style=\"width: ".$width."px;\">\n<img src=\"$name\" $sizexy alt=\"$alt\" title=\"$title\" />\n$selection</div>$preservep");
                                        }elsif($alignment eq 'right'){
                                                $textwindow->delete('thisblockstart','thisblockend');
                                                $textwindow->insert('thisblockstart',"<div class=\"figright\" style=\"width: ".$width."px;\">\n<img src=\"$name\" $sizexy alt=\"$alt\" title=\"$title\" />\n$selection</div>$preservep");
                                        }
                                        $textwindow->addGlobEnd;
                                        ...
                                        for($lglobal{alttext},$lglobal{titltext},$lglobal{widthent},$lglobal{heightent},$lglobal{imagelbl},$lglobal{imgname}){$_->destroy};
                                        $textwindow->tagRemove('highlight','1.0','end');
                ...
                $lglobal{htmlimpop}->protocol('WM_DELETE_WINDOW' => sub{
                        $lglobal{htmlthumb}->delete if $lglobal{htmlthumb};
                        $lglobal{htmlthumb}->destroy if $lglobal{htmlthumb};
                        $lglobal{htmlorig}->delete if $lglobal{htmlorig};
                        $lglobal{htmlorig}->destroy if $lglobal{htmlorig};
...
sub tnbrowse{
        ...
        $lglobal{imagelbl}->configure(-image => $lglobal{htmlthumb},
                -text => 'Thumbnail',
                -justify => 'center',
        );
...
sub linkcheck{
        if (defined($lglobal{lcpop})){
                ...
                my $frame = $lglobal{lcpop}->Frame->pack->pack(-anchor => 'nw', -expand => 'yes', -fill => 'both');
                $lglobal{linkchkbox} = $frame->Scrolled('Listbox',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => '{Courier} 10',
                        -height => 40,
                ...
                $lglobal{lcpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{lcpop}->destroy; undef $lglobal{lcpop}});
                $lglobal{lcpop}->Icon(-image => $icon);
        }
        BindMouseWheel($lglobal{linkchkbox});
        $lglobal{linkchkbox}->eventAdd('<<search>>' => '<ButtonRelease-2>','<ButtonRelease-3>');
        $lglobal{linkchkbox}->bind('<<search>>', sub {
                $lglobal{linkchkbox}->activate($lglobal{linkchkbox}->index
                ('@'.($lglobal{linkchkbox}->pointerx - $lglobal{linkchkbox}->rootx).','.($lglobal{linkchkbox}->pointery-$lglobal{linkchkbox}->rooty)));
                $lglobal{linkchkbox}->selectionClear(0,'end');
                $lglobal{linkchkbox}->selectionSet($lglobal{linkchkbox}->index('active'));
        ...
        $lglobal{linkchkbox}->bind('<<find>>', sub {
                my $sword = $lglobal{linkchkbox}->get('active');
                return unless (defined $sword and length $sword);
                        ...
                        if ($token->[0] eq 'T' and $token->[2]){
                                ...
                                for my $img (@urls){
                                        if ($img){
                                                ...
                                                delete $imagefiles{$img} if ((defined $imagefiles{$img})||(defined $link{$img})) ;
                                                push @warning, "WARNING! $img contains uppercase characters!" if ($img ne lc($img));
                                                push @warning, "CRITICAL! Image file: $img not found!" unless (-e $d.$img);
                                                $css++;
                                        }
                                }
                        }
        ...
        $lglobal{linkchkbox}->insert('end', "$ids unnamed anchors (tag with id attribute)");
        ...
        $lglobal{linkchkbox}->insert('end', 'INTERNAL LINKS WITHOUT ANCHORS. - (CRITICAL)','');
        for (natural_sort_alpha(keys %link)){
                unless ((defined $anchor{$_}) || (defined $id{$_}) || ($link{$_} eq $_)){
                        $lglobal{linkchkbox}->insert('end', "#$link{$_}");
                        $count++;
                };
        }
        $lglobal{linkchkbox}->insert('5', "$count internal links without anchors");
        $lglobal{linkchkbox}->insert('end', '','EXTERNAL LINKS. - (CRITICAL)','');
        my $externflag;
        for (natural_sort_alpha( keys %link )){
                if ($link{$_} eq $_){
                        if ($_ =~ /:\/\//){
                                ...
                                $temp =~ s/^([^#]+).*/$1/;
                                unless (-e $d.$temp){
                                        $lglobal{linkchkbox}->insert('end', "local file(s) not found!") unless $externflag;
                                        $lglobal{linkchkbox}->insert('end', "$link{$_}");
                                        $externflag++;
                                }
                        }
        ...
        $lglobal{linkchkbox}->insert('end', '','LINKS WITH BAD CHARACTERS. - (CRITICAL)','');
        for ( natural_sort_alpha( keys %badlink )){
                $lglobal{linkchkbox}->insert('end', "$badlink{$_}");
        }
        $lglobal{linkchkbox}->insert('end', '','IMAGE LINKS/FILES WITH PROBLEMS. - (CRITICAL)');
        $lglobal{linkchkbox}->insert('end', @warning) if @warning;
        $lglobal{linkchkbox}->insert('end', '');
        if (keys %imagefiles){
                for ( natural_sort_alpha( keys %imagefiles )){
                        $lglobal{linkchkbox}->insert('end', 'WARNING! File '.$_.' not used!')if ($_ =~ /\.(png|jpg|gif|bmp)/);
                }
                $lglobal{linkchkbox}->insert('end', '');
        }
        $lglobal{linkchkbox}->insert('end', '','ANCHORS WITHOUT LINKS. - (INFORMATIONAL)','');
...
sub htmlimages{
        ...
        return unless $start;
        $textwindow->see($start);
        my $end = $textwindow->search('-regexp','-count' => \$length, '--','\](<\/p>)?',$start,'end');
...
sub htmlautoconvert{
        ...
        while ($step <= $ler){
                ...
                if ($front){
                        ...
                        $textwindow->ntinsert("$step.0",'<p class="center">')if (length($selection)&&(!$last5[3])&&($selection !~ /<\/?h\d?|<br.*?>|<\/p>|<\/div>/));
                        $textwindow->ntinsert(($step-1).'.end','</p>')if (!(length($selection))&&($last5[3])&&($last5[3] !~ /<\/?h\d?|<br.*?>|<\/p>|<\/div>/));
                        push @last5, $selection;
                        shift @last5 while (scalar(@last5) > 4);
                        $step++;
                        next;
                ...
                if ($selection =~ /^\x7f*\/\#/){
                        ...
                        if (($last5[1])&&(!$last5[2])){
                                $textwindow->ntinsert(($step-3).".end",'</p>') unless ($textwindow->get(($step-3).'.0',($step-2).'.end') =~ /<\/?h\d?|<br.*?>|<\/p>|<\/div>/);
                        }
                        $textwindow->ntinsert(($step).".end",'</p>') unless (length $textwindow->get(($step+1).'.0',($step+1).'.end'));
                        push @last5, $selection;
                        shift @last5 while (scalar(@last5) > 4);
                        $step++;
                        next;
                ...
                if ($selection =~ /^\x7f*\/[\$\*]/){
                        $inblock = 1;
                        if (($last5[2])&&(!$last5[3])){
                                $textwindow->ntinsert(($step-2).'.end','</p>') unless (($textwindow->get(($step-2).'.0',($step-2).'.end') =~ /<\/?[hd]\d?|<br.*?>|<\/p>/));
                        }
                        $textwindow->replacewith("$step.0", "$step.end", '<p>');
                        $step++;
                        next;
                ...
                if (($last5[2])&&(!$last5[3])){
                        $textwindow->ntinsert(($step-2).'.end','</p>') unless (($textwindow->get(($step-2).'.0',($step-2).'.end') =~ /<\/?[hd]\d?|<br.*?>|<\/p>|<\/[uo]l>/)
                        ||($inblock));
                }
                if ($inblock || ($selection =~ /^\s/)){
                        if ($last5[3]){
                                if ($last5[3] =~ /^\S/){
                                        $last5[3] .= '<br />';
                                        $textwindow->ntdelete(($step-1).'.0',($step-1).'.end');
                        ...
                        if (($last5[2])&&(!$last5[3])&&($selection =~ /\/\*/)){
                                $textwindow->ntinsert(($step-2).".end",'</p>') unless ($textwindow->get(($step-2).'.0',($step-2).'.end') =~ /<\/[hd]\d?/);
                        }
                        push @last5, $selection;
                        shift @last5 while (scalar(@last5) > 4);
                        $step++;
                ...
                };
                {
                        ...
                        if ((!$last5[0])&&(!$last5[1])&&(!$last5[2])&&(!$last5[3])&&($selection)){
                                ...
                                $textwindow->ntinsert("$step.0","<h2><a name=\"".$aname."\" id=\"".$aname."\"></a>") unless ($selection =~ /<[ph]/);
                                $textwindow->ntinsert("$step.end",'</h2>') unless ($selection =~ /<[ph]/);
                                unless ($selection =~ /<p/){
                                        $selection =~ s/<sup>.*?<\/sup>//g;
                                        $selection =~ s/<[^>]+>//g;
                                        $selection = "<b>$selection</b>";
                                        push @contents, "<a href=\"#".$aname."\">".$selection."</a><br />\n" ;
                                }
                                $selection .= '<h2>';
                                $textwindow->see("$step.0");
                                $textwindow->update;
                        }elsif (($last5[2] =~ /<h2>/)&&($selection)){
                                $textwindow->ntinsert("$step.0",'<p>') unless (($selection =~ /<[pd]/)||($selection =~ /<[hb]r>/)||($inblock));
                        }elsif (($last5[2])&&(!$last5[3])&&($selection)){
                                $textwindow->ntinsert("$step.0",'<p>') unless (($selection =~ /<[phd]/)||($selection =~ /<[hb]r>/)||($inblock));
                        }elsif (($last5[1])&&(!$last5[2])&&(!$last5[3])&&($selection)){
                                $textwindow->ntinsert("$step.0",'<p>') unless (($selection =~ /<[phd]/)||($selection =~ /<[hb]r>/)||($inblock));
                        }elsif (($last5[0])&&(!$last5[1])&&(!$last5[2])&&(!$last5[3])&&($selection)){
                                $textwindow->ntinsert("$step.0",'<p>') unless (($selection =~ /<[phd]/)||($selection =~ /<[hb]r>/)||($inblock));
                        }
                }
                push @last5, $selection;
        ...
        push @contents,'</p>';
        working("Cleaning up\nblock Markers");
        while ($thisblockstart = $textwindow->search('-regexp', '--','^\/[\*\$\#]','1.0', 'end')){
        ...
        while ($thisblockstart = $textwindow->search('-regexp', '--','<\/h\d><br />','1.0', 'end')){
        ...
        $thisblockstart = '1.0';
        while ($thisblockstart = $textwindow->search('-exact', '--','<p>FOOTNOTES:',$thisblockstart, 'end')){
                ...
                $thisblockstart = $textwindow->search('-exact', '--','<hr',$thisblockstart, 'end');
                if ($thisblockstart){
        ...
        my $thisnoteend;
        my $length;
        while ($thisblockstart = $textwindow->search('-regexp', '-count' => \$length, '--','(<p>)?\[Sidenote:\s*','1.0', 'end')){
                ...
                $textwindow->ntdelete($thisnoteend,"$thisnoteend+5c") if $thisnoteend;
                ...
                for $mark(@marknames){
                        if($mark =~ /Pg(\w+)/){
                                ...
                                if ($check =~ /<h[12]>/){
                                        ...
                                        $textwindow->ntinsert($markindex,"<span class='pagenum'><a name=\"Page_$num\" id=\"Page_$num\">[Pg $num]</a></span>") if $lglobal{pageanch};
                                        #$textwindow->ntinsert($markindex,"<span class='pagenum' id=\"Page_".$num."\">[Pg $num]</span>") if $lglobal{pageanch};
                                $textwindow->ntinsert($markindex,'<!-- Page '.$num.' -->') if ($lglobal{pagecmt} and $num);
                                my $pstart = $textwindow->search('-backwards','-exact','--','<p>',$markindex,'1.0') || '1.0';
                                my $pend = $textwindow->search('-backwards','-exact','--','</p>',$markindex,'1.0') || '1.0';
                                my $sstart = $textwindow->search('-backwards','-exact','--','<div ',$markindex,'1.0') || '1.0';
                                my $send = $textwindow->search('-backwards','-exact','--','</div>',$markindex,$pend) || $pend;
                                if ($textwindow->compare($pend, '>=', $pstart)){
                                        $textwindow->ntinsert($markindex,'<p>') unless ($textwindow->compare($send, '<', $sstart));
                                }
                                my $anchorend = $textwindow->search('-exact','--',']</a></span>',$markindex,'end');
                                $anchorend = $textwindow->index("$anchorend+12c");
                                $pstart = $textwindow->search('-exact','--','<p>',$anchorend,'end') || 'end';
                                $pend = $textwindow->search('-exact','--','</p>',$anchorend,'end') || 'end';
                                $sstart = $textwindow->search('-exact','--','<div ',$anchorend,'end') || 'end';
                                $send = $textwindow->search('-exact','--','</div>',$anchorend, $sstart) || $sstart;
                                if ($textwindow->compare($pend, '>=', $pstart)){
                                        $textwindow->ntinsert($anchorend,'</p>') unless ($textwindow->compare($send, '<', $sstart));
                                }
                        }
                }
        }
        ...
        if ($lglobal{leave_utf}){
                $thisblockstart = $textwindow->search('-exact', '--','charset=iso-8859-1','1.0', 'end');
                if ($thisblockstart){
        ...
        unless ($lglobal{leave_utf}){
                while ($thisblockstart = $textwindow->search('-regexp', '--','[\x{100}-\x{65535}]','1.0', 'end')){
        ...
        for (reverse(sort(values(%{$lglobal{classhash}})))){$textwindow->ntinsert($thisblockstart.' +1l linestart',$_) if keys %{$lglobal{classhash}}};
        %{$lglobal{classhash}} = ();
        working();
        $textwindow->Unbusy;
        $textwindow->see('1.0');
...
sub fracconv{
        ...
        while (($ascii,$html) = each (%frachash)){
                while ($thisblockstart = $textwindow->search('-regexp', '-count' => \$length, '--',"-?$ascii", $start, $end)){
...
sub tonamed{
        ...
        if($range_total == 0){
                ...
                while (@ranges){
                        ...
                        while ($thisblockstart = $textwindow->search('-regexp', '--','[\x{100}-\x{65535}]',$start,'srchend')){
                                my $xchar = ord($textwindow->get($thisblockstart));
                                $textwindow->ntdelete($thisblockstart,"$thisblockstart+1c");
                                $textwindow->ntinsert($thisblockstart,"&#$xchar;");
                        }
...
sub fromnamed{
        ...
        if($range_total == 0){
                ...
                while (@ranges){
                        ...
                        while ($thisblockstart = $textwindow->search('-regexp', '-count' => \$length, '--','&#\d+;',$start,$end)){
...
sub named{
        ...
        $end = 'end' unless $end;
        $textwindow->markSet('srchend',$end);
        while ($searchstartindex = $textwindow->search('-regexp', '-count' => \$length, '--',$from,$searchstartindex,'srchend')){
                $textwindow->ntdelete($searchstartindex,$searchstartindex.'+'.$length.'c');
...
sub hilitepopup{
        ...
        if (defined($lglobal{hilitepop})){
                ...
                $f->Label(
                -text => 'Highlight Character(s) or Regex',
                )->pack(-side => 'top', -pady => 2, -padx => 2, -anchor => 'n');
                my $entry = $f->Entry(
                        -width => 40,
                        -background => 'white',
                        -font => $lglobal{font},
                        -relief => 'sunken',
                )->pack(-expand => 1, -fill => 'x', -padx => 3, -pady => 3, -anchor => 'n');
                ...
                $f3->Button(
                        -activebackground => $activecolor,
                        -command => sub {if ($textwindow->markExists('selstart')){
                ...
                $f3->Button(
                        -activebackground => $activecolor,
                        -command => sub {$textwindow->tagRemove('quotemark','1.0','end')},
                        -text => 'Remove Highlight',
                        -width => 16,
                )->grid(-row => 2, -column => 2, -padx => 2, -pady => 2);

                $lglobal{hilitepop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{hilitepop}->destroy; undef $lglobal{hilitepop}});
...
sub hilite{
        ...
        if($range_total == 0){
                ...
                my $length;
                while ($lastindex){
                        $index = $textwindow->search('-regexp', -count => \$length, '--',$mark,$lastindex,$thisblockend);
                        $textwindow->tagAdd('quotemark', $index, $index.' +'.$length.'c') if $index;
                        if ($index){$lastindex = "$index+1c"}else{$lastindex = ''} ;
                }
        }
}
...
sub seperatorpopup{
        ...
        if (defined($lglobal{pagepop})){
                ...
                my $sf1 = $lglobal{pagepop}->Frame->pack(-side => 'top', -anchor => 'n');
                ...
                my $jautobutton = $sf3->Checkbutton(
                        ...
                        -command => sub {       $lglobal{jsemiautomatic} = 0 if $lglobal{jsemiautomatic};       },
                        -selectcolor => $lglobal{checkcolor},
                        -text => 'Full Auto',
                )->pack(-side => 'left', -pady => 2, -padx => 2, -anchor => 'w');
                my $jsautobutton = $sf3->Checkbutton(
                        -variable => \$lglobal{jsemiautomatic},
                        -command => sub {$lglobal{jautomatic} = 0 if $lglobal{jautomatic}; },
                        -selectcolor => $lglobal{checkcolor},
                        -text => 'Semi Auto',
                )->pack(-side => 'left', -pady => 2, -padx => 2, -anchor => 'w');
                my $sf4 = $lglobal{pagepop}->Frame->pack(-side => 'top', -anchor => 'n', -padx => 5);
        ...
        $lglobal{pagepop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{pagepop}->destroy; undef $lglobal{pagepop}; $textwindow->tagRemove('highlight','1.0','end')});
        ...
        $lglobal{pagepop}->Tk::bind('<v>' => sub{ openpng(); $lglobal{pagepop}->raise;});
        $lglobal{pagepop}->Tk::bind('<u>' => \&undojoin);
        $lglobal{pagepop}->Tk::bind('<a>' => sub{if($lglobal{jautomatic}){$lglobal{jautomatic} = 0}else{$lglobal{jautomatic}=1}});
        $lglobal{pagepop}->Tk::bind('<s>' => sub{if($lglobal{jsemiautomatic}){$lglobal{jsemiautomatic} = 0}else{$lglobal{jsemiautomatic}=1}});
...
sub phelppopup{
        if (defined($lglobal{phelppop})){
                ...
                my $button_ok = $lglobal{phelppop}->Button(
                        -activebackground => $activecolor,
                        -text => 'OK',
                        -command => sub {$lglobal{phelppop}->destroy; undef $lglobal{phelppop}}
                )->pack;
                $lglobal{phelppop}->resizable('no','no');
                $lglobal{phelppop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{phelppop}->destroy; undef $lglobal{phelppop}});
...
sub convertfilnum{
        ...
        $searchstartindex = '1.0';
        $searchendindex = '1.0';
        $searchstartindex = $textwindow->search('-nocase', '-regexp','--', '^-----*\s?File:', $searchendindex, 'end');
        return unless $searchstartindex;
        $searchendindex = $textwindow->index("$searchstartindex lineend");
        $line = $textwindow->get($searchstartindex,$searchendindex);
        $textwindow->tagAdd('highlight',$searchstartindex,$searchendindex)if $searchstartindex;
        $textwindow->yview('end');
        $textwindow->see($searchstartindex)if $searchstartindex;
        if ($lglobal{jautomatic}&&$searchstartindex){
                ...
                if (($line =~ /\p{IsLower}/)||($line =~ /^I /)){joinlines('j')}
                my ($r,$c) = split /\./, $textwindow->index('page-1c');
                my ($size) = length($textwindow->get('page+1l linestart','page+1l lineend'));
                if (($line =~ /[\.\"\'\?]/)&&($c < ($size * 0.5))){joinlines('l')}
        }
        $textwindow->xviewMoveto(.0);
        $textwindow->markSet('insert',"$searchstartindex+2l") if $searchstartindex;
}
        ...
        if (defined($lglobal{prooferpop})){
                ...
                $bframe->Button(
                        ...
                        -command => sub{
                                ...
                                my $proofer = '';
                                if ($range_total){
                                        $proofer = $lglobal{prfrrotextbox}->get($ranges[0],$ranges[1]);
                                        $proofer =~ s/^\s+//;
                                        $proofer =~ s/\s\s.*//s;
                                        $proofer =~ s/\s/%20/g;
                                }
                ...
                for my $round ( 1..$lglobal{numrounds} ) {
                        $bframe->Button(
                                ...
                                -text => "Round $round",
                                -width => 12
                        )->grid(-row => (($round + 1) %2) +1 , -column => int (($round + 5) / 2), -padx => 3, -pady => 3);
                }
                my $frame = $lglobal{prooferpop}->Frame->pack
                        (-anchor => 'nw', -expand => 'yes', -fill => 'both');
                $lglobal{prfrrotextbox} = $frame->Scrolled('ROText',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => '{Courier} 10',
                        -width => 80,
                ...
                prfrbypage();
                $lglobal{prooferpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{prooferpop}->destroy; undef $lglobal{prooferpop}});
...
sub prfrhdr{
        my ($max) = @_;
        $lglobal{prfrrotextbox}->insert('end', sprintf ("%*s     ", (-$max),'   Name'));
        for ( 1.. $lglobal{numrounds}) {
            $lglobal{prfrrotextbox}->insert('end', sprintf (" %-8s", "Round $_"));
        }
...
sub prfrbypage{
        ...
        for my $round (1 .. $lglobal{numrounds}){
                $lglobal{prfrrotextbox}->insert('end', sprintf (" %*s",(-$max[$round]-2), "Round $round"));
        }
        ...
        for my $page(sort keys %proofers){
                        $lglobal{prfrrotextbox}->insert('end', sprintf ("%-8s", $page));
                        for my $round (1 .. $lglobal{numrounds}){
                                $lglobal{prfrrotextbox}->insert('end', sprintf (" %*s",(-$max[$round]-2), $proofers{$page}->[$round]||'<none>'));
                        }
                        $lglobal{prfrrotextbox}->insert('end',"\n");
        }
}
...
sub prfrbyname{
        ...
        for $page(keys %proofers){
                for (1..$lglobal{numrounds}){
                        $max = length $proofers{$page}->[$_] if ($proofers{$page}->[$_] and length $proofers{$page}->[$_] > $max);
                }
        }
        $lglobal{prfrrotextbox}->delete('1.0','end');
        foreach $page(keys %proofers){
                for (1..$lglobal{numrounds}){
                        $proofersort{$proofers{$page}->[$_]}[$_]++ if $proofers{$page}->[$_];
                        $proofersort{$proofers{$page}->[$_]}[0]++ if $proofers{$page}->[$_];
                }
        }
        prfrhdr($max);
        delete $proofersort{''};
        foreach $prfr ( sort {deaccent(lc($a)) cmp deaccent(lc($b))}(keys %proofersort)) {
                for (1..$lglobal{numrounds}){
                        $proofersort{$prfr}[$_] = "0" unless $proofersort{$prfr}[$_];
                }
                $lglobal{prfrrotextbox}->insert('end',sprintf ("%*s", (-$max-2), $prfr));
                for (1..$lglobal{numrounds}){
                        $lglobal{prfrrotextbox}->insert('end',sprintf (" %8s",$proofersort{$prfr}[$_]));
                }
                $lglobal{prfrrotextbox}->insert('end', sprintf (" %8s\n", $proofersort{$prfr}[0]));
        }
}
...
sub prfrby{
        ...
        delete $ptemp{''};
        prfrhdr($max);
        foreach $prfr (sort { $ptemp{$b} <=> $ptemp{$a} || ( deaccent(lc($a)) cmp deaccent(lc($b)) ) }keys %ptemp) {
                $lglobal{prfrrotextbox}->insert('end', sprintf ("%*s", (-$max-2), $prfr));
                for (1..$lglobal{numrounds}){
                        $lglobal{prfrrotextbox}->insert('end', sprintf (" %8s", $proofersort{$prfr}[$_]||'0'));
                }
                $lglobal{prfrrotextbox}->insert('end', sprintf (" %8s\n", $proofersort{$prfr}[0]));
        }
}
...
sub guesswindow{
        ...
        if ($lglobal{pgpop}){
                ...
                $f0->Label(
                        -text => 'This function should only be used if you have the page images but no page markers in the text.',
                )->grid(-row => 1, -column => 1, -padx => 1, -pady => 2);
                my $f1 = $lglobal{pgpop}->Frame->pack;
                $f1->Label(
                        -text => 'How many pages are there total?',
                )->grid(-row => 1, -column => 1, -padx => 1, -pady => 2);
                my $tpages = $f1->Entry(
                        -background => 'white',
                        -width => 8,
                ...
                $f1->Label(
                        -text => 'What line # does page 25 start with?',
                )->grid(-row => 2, -column => 1, -padx => 1, -pady => 2);
                my $page25 = $f1->Entry(
                        -background => 'white',
                        -width => 8,
                ...
                $lglobal{pgpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{pgpop}->destroy; undef $lglobal{pgpop}});
...
sub joinlines{
        ...
        $searchstartindex = '1.0';
        $searchendindex = '1.0';
        $searchstartindex = $textwindow->search('-regexp','--', '^-----*\s?File:', $searchendindex, 'end');
        unless ($searchstartindex){
                $textwindow->bell unless $nobell;
                return;
        }
        ...
        unless ($op eq 'd'){
                ...
                while (1){
                        ...
                        last if ($textwindow->compare($index, '>=', 'end'));
                        $line = $textwindow->get("$index-1c");
                        if ($line eq '*'){$line = $textwindow->get("$index-2c").'*'};
                        if (($line =~ /[\s\n]$/)||($line =~ /[\w-]\*$/)){
                                $textwindow->delete("$index-1c");
                                $lglobal{joinundo}++;
                        }else{
        ...
        if ($op eq 'j'){
                ...
                if ($line =~ /\-/){
                        unless ($textwindow->search('-regexp','--', '-----*\s?File:', $index,"$index lineend")){
                                while ($line =~ /\-/){
                                        $textwindow->delete("$index-1c");
                                        $lglobal{joinundo}++;
                                        $index = $textwindow->index('page');
                                ...
                                $line = $textwindow->get($index);
                                if ($line =~ /\*/){$textwindow->delete($index); $lglobal{joinundo}++; };
...
sub undojoin{
        if ($lglobal{jautomatic}){$textwindow->undo; $textwindow->tagRemove('highlight','1.0','end');return};
        my $index;
...
sub tidypop_up{
        ...
        if ($lglobal{tidypop}){
                ...
                my $opsbutton = $ptopframe->Button(
                        ...
                        -text => 'Get Errors',
                        -width => 16
                )->pack(-side => 'left', -pady => 10, -padx => 2, -anchor => 'n');
                ...
                my $pframe = $lglobal{tidypop}->Frame->pack(-fill => 'both', -expand => 'both',);
                $lglobal{tidylistbox} = $pframe->Scrolled('Listbox',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => $lglobal{font},
                        -selectmode => 'single',
                ...
                $lglobal{tidypop}->protocol('WM_DELETE_WINDOW' => sub{
                ...
                $lglobal{tidypop}->bind('<Configure>' => sub {
                        $lglobal{tidypop}->XEvent;
                        $geometry2 = $lglobal{tidypop}->geometry;
                        $lglobal{geometryupdate} = 1;
                        }
                );
                $lglobal{tidylistbox}->eventAdd('<<remove>>' => '<ButtonRelease-2>','<ButtonRelease-3>');
                $lglobal{tidylistbox}->bind('<<remove>>', sub {
                        $lglobal{tidylistbox}->activate($lglobal{tidylistbox}->index
                        ('@'.($lglobal{tidylistbox}->pointerx - $lglobal{tidylistbox}->rootx).','.($lglobal{tidylistbox}->pointery-$lglobal{tidylistbox}->rooty)));
                        $lglobal{tidylistbox}->selectionClear(0,'end');
                        $lglobal{tidylistbox}->selectionSet($lglobal{tidylistbox}->index('active'));
...
sub tidyrun{
        ...
        if (open my $td, '>', $name){
                ...
                my $dialog = $top->Dialog(
                        -text => 'Could not write to the '.cwd().' directory. Check for write permission or space problems.',
                        -bitmap => 'question',
                        -title => 'Tidy problem',
                        -buttons => [qw/OK/],
                );
        ...
        $top->Unbusy;
        $lglobal{tidylistbox}->insert('end',"Tidied file written to $name") if ($tidyoptions =~ /\-m/);
        unlink 'tidy.tmp';
        tidypop_up();
}

sub gutopts{
        my $gcdialog = $top->DialogBox(-title => 'Gutcheck Options', -buttons => ['OK']);
        my $gcopt6 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[6],
                -selectcolor => $lglobal{checkcolor},
                -text => '-v Enable verbose mode (Recommended).',
        )->pack(-side => 'top', -anchor => 'nw', -padx => 5);
        my $gcopt0 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[0],
                -selectcolor => $lglobal{checkcolor},
                -text => '-t Disable check for common typos.',
        )->pack(-side => 'top', -anchor => 'nw', -padx => 5);
        my $gcopt1 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[1],
                -selectcolor => $lglobal{checkcolor},
                -text => '-x Disable paranoid mode.',
        )->pack(-side => 'top', -anchor => 'nw', -padx => 5);
        my $gcopt2 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[2],
                -selectcolor => $lglobal{checkcolor},
                -text => '-p Report ALL unbalanced double quotes.',
        )->pack(-side => 'top', -anchor => 'nw', -padx => 5);
        my $gcopt3 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[3],
                -selectcolor => $lglobal{checkcolor},
                -text => '-s Report ALL unbalanced single quotes.',
        )->pack(-side => 'top', -anchor => 'nw', -padx => 5);
        my $gcopt4 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[4],
                -selectcolor => $lglobal{checkcolor},
                -text => '-m Interpret HTML markup.',
        )->pack(-side => 'top', -anchor => 'nw', -padx => 5);
        my $gcopt5 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[5],
                -selectcolor => $lglobal{checkcolor},
                -text => '-l Do not report non DOS newlines.',
        )->pack(-side => 'top', -anchor => 'nw', -padx => 5);
        my $gcopt7 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[7],
                -selectcolor => $lglobal{checkcolor},
                -text => '-u Flag words from the .typ file.',
        )->pack(-side => 'top', -anchor => 'nw', -padx => 5);
        my $gcopt8 = $gcdialog->add('Checkbutton',
                -variable => \$gcopt[8],
                -selectcolor => $lglobal{checkcolor},
...
sub gutcheck{
        ...
        unless ($gutpath){ $gutpath = $textwindow->getOpenFile(-filetypes => $types, -title => 'Where is the Gutcheck executable?')};
        return unless $gutpath;
        my $gutcheckoptions = ' -ey';   # e - echo queried line. y - puts errors to stdout instead of stderr.
        if ($gcopt[0]){$gutcheckoptions .= 't'}; # Check common typos
        if ($gcopt[1]){$gutcheckoptions .= 'x'}; # "Trust no one" Paranoid mode. Queries everything
        if ($gcopt[2]){$gutcheckoptions .= 'p'}; # Require closure of quotes on every paragraph
        if ($gcopt[3]){$gutcheckoptions .= 's'}; # Force checking for matched pairs of single quotes
        if ($gcopt[4]){$gutcheckoptions .= 'm'}; # Ignore markup in < >
        if ($gcopt[5]){$gutcheckoptions .= 'l'}; # Line end checking - defaults on
        if ($gcopt[6]){$gutcheckoptions .= 'v'}; # Verbose - list EVERYTHING!
        if ($gcopt[7]){$gutcheckoptions .= 'u'}; # Use file of User-defined Typos
...
sub gcheckpop_up{
        ...
        if ($lglobal{gcpop}){
                ...
                $lglobal{gclistbox} = $pframe->Scrolled('Listbox',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => $lglobal{font},
                        -selectmode => 'single',
                ...
                $lglobal{gcpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{viewpop}->iconify if defined $lglobal{viewpop}; $lglobal{gcpop}->destroy; undef $lglobal{gcpop};
                ...
                $lglobal{gcpop}->bind('<Configure>' => sub {
                        $lglobal{gcpop}->XEvent;
                        $geometry2 = $lglobal{gcpop}->geometry;
                        $lglobal{geometryupdate} = 1;
                        }
                );
                $lglobal{gclistbox}->eventAdd('<<remove>>' => '<ButtonRelease-2>','<ButtonRelease-3>');
                $lglobal{gclistbox}->bind('<<remove>>', sub {
                        $lglobal{gclistbox}->activate($lglobal{gclistbox}->index
                        ('@'.($lglobal{gclistbox}->pointerx - $lglobal{gclistbox}->rootx).','.($lglobal{gclistbox}->pointery-$lglobal{gclistbox}->rooty)));
                ...
                $lglobal{gclistbox}->bind('<Button-3>', sub {
                        $lglobal{gclistbox}->activate($lglobal{gclistbox}->index
                        ('@'.($lglobal{gclistbox}->pointerx - $lglobal{gclistbox}->rootx).','.($lglobal{gclistbox}->pointery-$lglobal{gclistbox}->rooty)));
                });
        }
        ...
        unless (open $results, '<', 'gutrslts.txt'){
                my $dialog = $top->Dialog(
                        -text => 'Could not read gutcheck results file. Problem with gutcheck.',
                        -bitmap => 'question',
                        -title => 'Gutcheck problem',
                        -buttons => [qw/OK/],
                );
                ...
                if ($line =~ /Line (\d+)/){
                        ...
                        if ($line =~ /Line \d+ column (\d+)/){
                                ...
                                if ($line =~ /Query standalone (\d)/){
                                        $word = '(?<=\D)'.$1.'(?=\D)';
                                        $lincol = $textwindow->search('-regexp','--',$word, "$linenum.0", "$linenum.0 +1l");
                                }
                                if ($line =~ /Asterisk?/){
                                        $lincol = $textwindow->search('--','*', "$linenum.0", "$linenum.0 +1l");
                                }
                                if ($line =~ /Hyphen at end of line?/){
                                        $lincol = $textwindow->search('-regexp','--','-$',"$linenum.0", "$linenum.0 +1l");
                                }
                                ...
                                if ($line =~ /Query word ([\p{Alnum}']+)/){
                                        $word = $1;
                                        if ($word =~ /[\xA0-\xFF]/){
                                                $lincol = $textwindow->search('-regexp','--','(?<!\p{Alnum})'.$word.'(?!\p{Alnum})', "$linenum.0", "$linenum.0 +1l");
                                        }elsif($word eq 'i'){
                                                $lincol = $textwindow->search('-regexp','--',' '.$word.'[^a-z]', "$linenum.0", "$linenum.0 +1l");
                                                $lincol = $textwindow->search('-regexp','--','[^A-Za-z0-9<\/]'.$word.'[^A-Za-z0-9>]', "$linenum.0", "$linenum.0 +1l") unless $lincol;
                                                $lincol = $textwindow->index("$lincol +1c") if ($lincol);
                                        }else{
                                                $lincol = $textwindow->search('-regexp','--','\b'.$word.'\b', "$linenum.0", "$linenum.0 +1l");
                                        }
                                        }
                                if ($line =~ /Query he\/be/){
                                        $lincol = $textwindow->search('-regexp','--','(?<= )[bh]e\W', "$linenum.0", "$linenum.0 +1l");
                                }
                                if ($line =~ /Query hut\/but/){
                                        $lincol = $textwindow->search('-regexp','--','(?<= )[bh]ut\W', "$linenum.0", "$linenum.0 +1l");
                                }
                        }
                        $mark++;
                        if ($lincol){
...
sub gcviewops{
        ...
        if (defined($lglobal{viewpop})){
                ...
                $pframe->Label(
                        -text => 'Select option to hide that error.',
                )->pack;
                ...
                $lglobal{viewpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{viewpop}->destroy; @{$lglobal{gcarray}} = (); undef $lglobal{viewpop}});
...
sub fixpopup{
        ...
        if (defined($lglobal{fixpop})){
                ...
                $tframe->Button(
                        -activebackground => $activecolor,
                        -command => sub{$lglobal{fixpop}->UnmapWindow; fixup(); $lglobal{fixpop}->destroy; undef $lglobal{fixpop}},
                        -text => 'Go!',
                        -width => 14
                )->pack(-pady => 6);
                my $pframe = $lglobal{fixpop}->Frame->pack;
                $pframe->Label(
                        -text => 'Select options for the fixup routine.',
                )->pack;
                ...
                $lglobal{fixpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{fixpop}->destroy; undef $lglobal{fixpop}});
...
sub fixup{
        ...
        while ($lastindex < $end){
                ...
                unless($inblock&&${$lglobal{fixopt}}[0]){
                        if (${$lglobal{fixopt}}[10]){while($line =~ s/(?<=\S)\s\s+(?=\S)/ /){$edited++}};
                        if (${$lglobal{fixopt}}[12]){$edited++ if $line =~ s/llth/11th/g;
                                $edited++ if $line =~ s/(?<=\d)lst/1st/g;
                                $edited++ if $line =~ s/(?<=\s)lst/1st/g;
                                $edited++ if $line =~ s/^lst/1st/;
                        }
                        if (${$lglobal{fixopt}}[1]){$edited++ if $line =~ s/ -/-/g;                             # Remove space before hyphen
                        ...
                        if (${$lglobal{fixopt}}[2]){$edited++ if $line =~ s/ +\.(?=\D)/\./g};# Get rid of space before periods
                        if (${$lglobal{fixopt}}[3]){$edited++ if $line =~ s/ +!/!/g};           # Get rid of space before exclamation points
                        if (${$lglobal{fixopt}}[4]){$edited++ if $line =~ s/ +\?/\?/g};         # Get rid of space before question marks
                        if (${$lglobal{fixopt}}[5]){$edited++ if $line =~ s/ +\;/\;/g};         # Get rid of space before semicolons
                        if (${$lglobal{fixopt}}[6]){$edited++ if $line =~ s/ +:/:/g};           # Get rid of space before colons
                        if (${$lglobal{fixopt}}[7]){$edited++ if $line =~ s/ +,/,/g};           # Get rid of space before commas
                        if (${$lglobal{fixopt}}[8]){$edited++ if $line =~ s/^\" +/\"/;          # Remove space after doublequote if it is the first character on a line
                                $edited++ if $line =~ s/ +\"$/\"/;                      # Remove space before doublequote if it is the last character on a line
                        }
                        if (${$lglobal{fixopt}}[9]){$edited++ if $line =~ s/(?<=(\(|\{|\[)) //g;        # Get rid of space after opening brackets
                                $edited++ if $line =~ s/ (?=(\)|\}|\]))//g;             # Get rid of space before closing brackets
                        }
                        if (${$lglobal{fixopt}}[13]){$edited++ if $line =~ s/(?<![\.\!\?])\.{3}(?!\.)/ \.\.\./g;
                                $edited++ if $line =~ s/^ \./\./;
                        }
                        if (${$lglobal{fixopt}}[11]){$edited++ if $line =~ s/^\s*(\*\s*){5}$/       \*       \*       \*       \*       \*\n/};
                        $edited++ if ($line =~ s/ +$//);
                        if (${$lglobal{fixopt}}[14] and ${$lglobal{fixopt}}[15]){$edited++ if $line =~ s/«\s+/«/g; $edited++ if $line =~ s/\s+»/»/g};
                        if (${$lglobal{fixopt}}[14] and !${$lglobal{fixopt}}[15]){$edited++ if $line =~ s/\s+«/«/g; $edited++ if $line =~ s/»\s+/»/g};
...
sub wordcount{
        ...
        if ($lglobal{popup}){
                ...
                $lglobal{popup}->title('Word frequency - Ctrl+s to save, Ctrl+x to export');
                $lglobal{popup}->geometry($geometry2) if $geometry2;
                my $wcseframe = $lglobal{popup}->Frame->pack(-side => 'top', -anchor => 'n');
                ...
                $wcseframe->Button(
                        -activebackground => $activecolor,
                        -command => sub{return unless ($lglobal{wclistbox}->curselection); $lglobal{harmonics} = 1; harmonicspop()},
                        -text => '1st Harm',
                )->pack(-side => 'left', -padx => 1, -pady => 1, -anchor => 'nw');
                $wcseframe->Button(
                        -activebackground => $activecolor,
                        -command => sub{return unless ($lglobal{wclistbox}->curselection); $lglobal{harmonics} = 2; harmonicspop()},
                        -text => '2nd Harm',
                )->pack(-side => 'left', -padx => 1, -pady => 1, -anchor => 'nw');
                $wcseframe->Button(
                        -activebackground => $activecolor,
                        ...
                        -text => 'Re Run ',
                )->pack(-side => 'left', -padx => 2, -pady => 1, -anchor => 'nw');
                my $wcseframe1 = $lglobal{popup}->Frame->pack(-side => 'top', -anchor => 'n');
                my @wfbuttons = (
                        ['Emdashes' => \&dashcheck],
                        ['Hyphens' => \&hyphencheck],
                        ['Alpha/num' => \&alphanumcheck],
                        ['All Words' => sub{
                                $lglobal{saveheader} = "$wc total words. ".keys(%{$lglobal{seen}})." distinct words in file.";
                ...
                my $wcframe = $lglobal{popup}->Frame->pack(-fill => 'both', -expand => 'both',);
                $lglobal{wclistbox} = $wcframe->Scrolled('Listbox',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => $lglobal{font},
                        -selectmode => 'single',
                ...
                $lglobal{popup}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{popup}->destroy; undef $lglobal{popup}; undef $lglobal{wclistbox}});
                $lglobal{popup}->Icon(-image => $icon);
                BindMouseWheel($lglobal{wclistbox});
                $lglobal{wclistbox}->eventAdd('<<search>>' => '<ButtonRelease-3>');
                $lglobal{wclistbox}->bind('<<search>>', sub {
                                $lglobal{wclistbox}->selectionClear(0,'end');
                                $lglobal{wclistbox}->selectionSet($lglobal{wclistbox}->index
                                ('@'.($lglobal{wclistbox}->pointerx - $lglobal{wclistbox}->rootx).','.($lglobal{wclistbox}->pointery-$lglobal{wclistbox}->rooty)));
                ...
                $lglobal{wclistbox}->bind('<<find>>', sub {
                                my ($sword) = $lglobal{wclistbox}->get($lglobal{wclistbox}->curselection);
                                ...
                                if ($sword =~ /\W/){
                                        ...
                                        $sword =~ s/\*space\*/ /;
                                        $sword =~ s/([^\w\s\\])/\\$1/g;
                                        $sword .= '\b' if ((length $sword gt 1)&&( $sword =~ /\w$/));
                                        searchoptset(qw/0 x x 1/);
                                }
                                ...
                                unless ($snum){
                                        searchoptset(qw/0 x x 1/);
                                        unless ($sword =~ m/--/){$sword = "(?<=-)$sword|$sword(?=-)"};
                                }
                                searchtext($sword);
                                searchoptset(@savesets);
                                $top->raise;
                ...
                $lglobal{wclistbox}->bind('<<harm>>', sub {return unless ($lglobal{wclistbox}->curselection); harmonics($lglobal{wclistbox}->get('active')); harmonicspop()});
                $lglobal{wclistbox}->eventAdd('<<adddict>>' => '<Control-Button-2>','<Control-Button-3>');
                $lglobal{wclistbox}->bind('<<adddict>>', sub {
                                ...
                                $lglobal{saveheader} = scalar(keys %{$lglobal{spellsort}}).' words not recognised by the spellchecker.';
                                sortwords(\%{$lglobal{spellsort}});
                        }
                );
                $lglobal{popup}->bind('<Configure>' => sub {
                                $lglobal{popup}->XEvent;
                                $geometry2 = $lglobal{popup}->geometry;
                                $lglobal{geometryupdate} = 1;
                        }
                );
                $lglobal{wclistbox}->eventAdd('<<pnext>>' => '<Next>','<Prior>','<Up>','<Down>');
                $lglobal{wclistbox}->bind('<<pnext>>', sub {
                                $lglobal{wclistbox}->selectionClear(0,'end');
                                $lglobal{wclistbox}->selectionSet($lglobal{wclistbox}->index('active'));
                        }
                );
                $lglobal{wclistbox}->bind('<Home>', sub {
                                $lglobal{wclistbox}->selectionClear(0,'end');
                ...
                $lglobal{wclistbox}->bind('<End>', sub {
                                $lglobal{wclistbox}->selectionClear(0,'end');
                                $lglobal{wclistbox}->see($lglobal{wclistbox}->index('end'));
                                $lglobal{wclistbox}->selectionSet($lglobal{wclistbox}->index('end')-1);
                                $lglobal{wclistbox}->activate($lglobal{wclistbox}->index('end')-1);
                        }
                );
                $lglobal{popup}->bind('<Control-s>' => sub {
                                my ($name);
                                $name = $textwindow->getSaveFile(-title => 'Save Word Frequency List As', -initialdir => $globallastpath,
                ...
                $lglobal{popup}->bind('<Control-x>' => sub {
                                my ($name);
                                $name = $textwindow->getSaveFile(-title => 'Export Word Frequency List As', -initialdir => $globallastpath,
        ...
        savefile() if (($textwindow->FileName) && ($textwindow->numberChanges != 0));
        open my $fh, '<', $filename;
        while (my $line = <$fh>){
                utf8::decode($line);
...
sub ital_adjust{
        ...
        $f1->Entry(
                ...
                -validate => 'key',
                -vcmd => sub {return 1 unless $_[1]; return 1 unless ($_[1] =~ /\D/); return 0;},
        )->grid(-row => 1, -column => 1, -padx => 2, -pady => 4);
        $f1->Button(
                -activebackground => $activecolor,
                -command => sub {
        ...
        $markuppop->protocol('WM_DELETE_WINDOW' => sub{$markuppop->destroy; undef $markuppop;});
...
sub searchoptset{
        ...
        for (0..3){
                if (defined($lglobal{search})){
                        if ($opt[$_] !~ /[a-zA-Z]/){$opt[$_] ? $lglobal{"searchop$_"}->select : $lglobal{"searchop$_"}->deselect};
                }else{
...
sub harmonicspop{
        ...
        if ($lglobal{hpopup}){
                ...
                $lglobal{hlistbox} = $frame->Scrolled('Listbox',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => $lglobal{font},
                        -selectmode => 'single',
                ...
                $lglobal{hpopup}->protocol('WM_DELETE_WINDOW' => sub{
                ...
                $lglobal{hpopup}->bind('<Configure>' => sub {
                ...
                $lglobal{hlistbox}->bind('<<search>>', sub {
                                $lglobal{hlistbox}->selectionClear(0,'end');
                                $lglobal{hlistbox}->selectionSet($lglobal{hlistbox}->index
                                ('@'.($lglobal{hlistbox}->pointerx - $lglobal{hlistbox}->rootx).','.($lglobal{hlistbox}->pointery-$lglobal{hlistbox}->rooty)));
                ...
                $lglobal{hlistbox}->bind('<<find>>', sub {
                ...
                $lglobal{hlistbox}->bind('<Down>', sub{
                ...
                $lglobal{hlistbox}->bind('<<harm>>', sub {return unless ($lglobal{hlistbox}->curselection); harmonics($lglobal{hlistbox}->get('active'));harmonicspop();$geometry3 = $lglobal{hpopup}->geometry});
        }
        ...
        foreach $word ( sort {deaccent(lc $a) cmp deaccent(lc $b) }(keys %{$lglobal{harmonic}})) {
                $line = sprintf ("%-8d %s", $lglobal{seen}->{$word}, $word);    # Print to the file
...
sub itwords{
        ...
        for my $wordwo(keys %words){
                ...
                $display{$wordwo} = $display{$wordwo}-$words{$wordwo} if (($words{$wordwo})||($display{$wordwo} =~ /\\n/));
                delete $display{$wordwo} unless $display{$wordwo}
        }
        $suspects = (scalar keys %display)-$wordw;
        $lglobal{saveheader} = "$wordw words/phrases with markup, $suspects similar without. (\\n means newline)";
        $wholefile = ();
        sortwords(\%display);
        $top->Unbusy;
...
sub hyphencheck{
        ...
        foreach my $word(keys %{$lglobal{seen}}) {
                ...
                if ($word =~ /-/) {
                        $wordw++;
                        my $wordtemp = $word;
                        $display{$word} = $lglobal{seen}->{$word} unless $lglobal{suspects_only};
                        $word =~ s/-/--/g;
                        if ($lglobal{seenm}->{$word}){
                                $display{$wordtemp} = $lglobal{seen}->{$wordtemp} if $lglobal{suspects_only};
                                my $aword = $word.' ****';
...
sub dashcheck{
        ...
        foreach my $word(keys %{$lglobal{seenm}}) {
                ...
                if ($word =~ /-/) {
                        $wordw++;
                        my $wordtemp = $word;
                        $display{$word} = $lglobal{seenm}->{$word} unless $lglobal{suspects_only};
                        $word =~ s/--/-/g;
                        if ($lglobal{seen}->{$word}){
                                my $aword = $word.' ****';
                                $display{$wordtemp} = $lglobal{seenm}->{$wordtemp} if $lglobal{suspects_only};
                                $display{$aword} = $lglobal{seen}->{$word};
                                $wordwo++;
                        }
                }
...
sub wfspellcheck{
        ...
        if ($words){
                utf8::decode($words);
                open (my $file, ">:bytes", "checkfil.chk");                     #save it to a file temporarily
                print $file $words;
                close $file;
                my $spellopt = get_spellchecker_version() lt "0.6" ? "-l " : "list ";
                $spellopt .= "-d $globalspelldictopt" if $globalspelldictopt;
                my @templist = `$lglobal{spellexename} $spellopt < "checkfil.chk"`;     # feed the text to aspell, get an array of misspelled words out
                chomp @templist;                        # get rid of any newlines
...
sub accentcheck{
        ...
        my $wordwo = 0;
        foreach my $word (keys %{$lglobal{seen}}) {
                if ($word =~ /[\xC0-\xCF\xD1-\xD6\xD9-\xDD\xE0-\xEF\xF1-\xF6\xF9-\xFD]/){
                        $wordw++;
                        my $wordtemp = $word;
                        $display{$word} = $lglobal{seen}->{$word} unless $lglobal{suspects_only};
                        ...
                        for my $wordd(@dwords){
                                my $line = sprintf ("%-8d %s", $lglobal{seen}->{$wordd}, $wordd) if $lglobal{seen}->{$wordd};
...
sub confirmdiscard{
        if($textwindow->numberChanges){
                my $ans = $top->messageBox(-icon  => 'warning',
                        ...
                        -message => 'The text has been modified without being saved. Save edits?'
                );
                if ($ans =~ /yes/i){
                        savefile()
                }
...
sub setbookmark{
        ...
        if ($bookmarks[$bookmark]){$textwindow->tagRemove('bkmk',$indexb,"$indexb+1c")};
        if ($index ne $indexb){
...
sub gotobookmark{
        ...
        $textwindow->markSet('insert',"bkmk$bookmark") if $bookmarks[$bookmark];
        update_indicators();
        $textwindow->tagAdd('bkmk',"bkmk$bookmark","bkmk$bookmark+1c") if $bookmarks[$bookmark];
}
...
sub latinpopup{
        if (defined($lglobal{latinpop})){
                ...
                $lglobal{latinpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{latinpop}->destroy; undef $lglobal{latinpop}});
...
sub BindMouseWheel {
        my($w) = @_;
        if (OS_Win){
                $w->bind('<MouseWheel>' => [ sub { $_[0]->yview('scroll', -($_[1] / 120) * 3, 'units')},
                Ev('D') ]);
        }else{
                $w->bind('<4>' => sub {
...
sub working{
        ...
        if (defined($lglobal{workpop})&&(defined $msg)){
                ...
                $lglobal{workpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{workpop}->destroy; undef $lglobal{workpop}});
...
sub handleDND {
        ...
        eval {                                  # In case of an error, do the SelectionGet in an eval block
                if (OS_Win) {
                        $filename = $textwindow->SelectionGet(-selection => $sel, 'STRING');
                }else{
                        $filename = $textwindow->SelectionGet(-selection => $sel, 'FILE_NAME');
                }
        };
...
sub utfpopup{
        ...
        $usel->select;
        $fontlist->insert ('end',sort($textwindow->fontFamilies));
        $pframe = $utfpop->Frame(-background => 'white')->pack(-expand => 'y',-fill => 'both');
        $frame = $pframe->Scrolled('Pane',-background => 'white',-scrollbars => 'se', -sticky => 'nswe')->pack(-expand => 'y',-fill => 'both');
        drag($frame);
        for my $y(0 .. $rows){
                for my $x(0..15){
                        ...
                        $buttons[($y * 16) + $x]->bind('<ButtonPress-3>', sub{
        ...
        $utfpop->protocol('WM_DELETE_WINDOW' => sub{$blln->destroy; $utfpop->destroy;});
...
sub insertit{
        ...
        $lglobal{hasfocus}->markSet('insert',$spot.'+'.length($letter).'c') if $isatext;
}
...
sub regexref{
        if (defined($lglobal{regexrefpop})){
                ...
                my $button_ok = $lglobal{regexrefpop}->Button(
                        -activebackground => $activecolor,
                        -text => 'Close',
                        -command => sub {$lglobal{regexrefpop}->destroy; undef $lglobal{regexrefpop}}
                )->pack(-pady => 6);
                my $regtext = $lglobal{regexrefpop}->Scrolled('ROText',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => $lglobal{font},
                )->pack(-anchor => 'n', -expand => 'y', -fill => 'both');
                drag($regtext);
                $lglobal{regexrefpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{regexrefpop}->destroy; undef $lglobal{regexrefpop}});
...
sub tablefx{
        ...
        if (defined($lglobal{tblfxpop})){
                        ...
                        'Delete Sel. Line' => sub {
                                        ...
                                        if($range_total == 0){
                                                ...
                                                while (@ranges){
                                                        my $end = pop(@ranges);
                                                        my $start = pop(@ranges);
                                                        $textwindow->delete($start,$end)if ($textwindow->get($start) eq '|');;
                                                }
                                        }
                ...
                $lglobal{tblfxpop}->bind('<Control-z>', sub {$textwindow->undo; $textwindow->tagRemove('highlight','1.0','end');});
                $lglobal{tblfxpop}->bind('<Delete>', sub {tlineremove()});
                tblselect();
        }
        $lglobal{tblfxpop}->protocol('WM_DELETE_WINDOW' => sub{
                $textwindow->tagRemove('table','1.0','end');
                $textwindow->tagRemove('linesel','1.0','end');
                $textwindow->markUnset('tblstart','tblend');
...
sub tlineremove{
        ...
        $textwindow->addGlobEnd;
                ...
                while (@ranges){
                        my $end = pop(@ranges);
                        my $start = pop(@ranges);
                        $textwindow->replacewith($start,$end,' ')if ($textwindow->get($start) eq '|');
                }
        }
        $textwindow->tagAdd('table','tblstart','tblend');
        $textwindow->tagRemove('linesel','1.0','end');
...
sub tlineselect{
        ...
        if($range_total == 0){
                ...
                if ($op and ($op eq 'p')){
                        ...
                        $nextcolumn = $textwindow->search('-backward', '-exact', '--', '|', 'insert', 'insert linestart');
                }else{
...
sub tblspace{
        ...
        if($range_total == 0){
                ...
                $tline =~ y/|/ /c;
                while ($erow >= $srow){
                        $textwindow->insert("$erow.end","\n$tline") if length ($textwindow->get("$erow.0", "$erow.end"));
                        $erow--;
                }
                $textwindow->tagAdd('table','tblstart','tblend');
                $textwindow->tagRemove('linesel','1.0','end');
...
sub insertline{
        ...
        if($range_total == 0){
                ...
                for ($srow..$erow){
                        ...
                        if ($op eq 'a'){
                                $textwindow->delete("$_.$col") if ($textwindow->get("$_.$col") =~ /[ |]/);
                        };
...
sub coladjust{
        ...
        if ($lglobal{tblrwcol}){
                ...
                my $blankline;
                for (@table){
                        my $cell = substr($_,($col[($colindex-1)]),($col[$colindex]-$col[($colindex-1)]-1),'');
                        unless ($blankline){
                                $blankline = $_ if (($_ =~ /^[ |]+$/)&&($_ =~ /\|/));
                        }
                        $cell .= ' ';
                ...
                for my $cellcnt(@cells){
                        ...
                        my $wrapped = wrapper(0,0,($col[$colindex]-$col[($colindex-1)]+$dir),$templine);
                        push @tblwr, $wrapped;
                }
                my $rowcount = 0;
                $cellheight = 0;
                ...
                if ($dir > 0){
                        ...
                        for ($srow..$erow){
                                return 0 if ($textwindow->get("$_.@{[$lglobal{selectedline}-1]}") ne ' ');
                        }
                        for ($srow..$erow){
...
sub grid2step{
        ...
        $lglobal{stepmaxwidth} = 70 if (($lglobal{stepmaxwidth} =~ /\D/)||($lglobal{stepmaxwidth} < 15));
        my $selection = $textwindow->get('tblstart','tblend');
        $selection =~ s/\n +/\n/g;
        ...
        for my $row (0..$#tbl){
                for (0..$cols){
                        my $wrapped;
                        $wrapped = wrapper(($cell*5),($cell*5),$lglobal{stepmaxwidth},$tbl[$row][$_]) if $tbl[$row][$_];
                        $wrapped = " \n" unless $wrapped;
                        my @temparray = split(/\n/,$wrapped);
                        if ($cell){
                                for (@temparray){
...
sub textbindings{
        ...
        $textwindow->tagConfigure('pagenum', -background => 'yellow', -relief => 'raised',-borderwidth => 2);
        ...
        $textwindow->bind('TextUnicode', '<Control-S>' => \&savefile);
        $textwindow->bind('TextUnicode', '<Control-a>' => sub{$textwindow->selectAll});
        $textwindow->bind('TextUnicode', '<Control-A>' => sub{$textwindow->selectAll});
        $textwindow->eventAdd('<<Copy>>' => '<Control-C>','<Control-c>','<F1>');
        $textwindow->bind('TextUnicode', '<<Copy>>' => \&copy);
        $textwindow->eventAdd('<<Cut>>' => '<Control-X>','<Control-x>','<F2>');
        $textwindow->bind('TextUnicode', '<<Cut>>' => sub{cut()});

        $textwindow->bind('TextUnicode', '<Control-V>' => sub{paste()});
        $textwindow->bind('TextUnicode', '<Control-v>' => sub{paste()});
        $textwindow->bind('TextUnicode', '<F3>' => sub{
                        $textwindow->addGlobStart;
                        $textwindow->clipboardColumnPaste;
                        $textwindow->addGlobEnd;
                }
        );
        $textwindow->bind('TextUnicode', '<Control-quoteleft>' => sub{
                        $textwindow->addGlobStart;
                        $textwindow->clipboardColumnPaste;
                        $textwindow->addGlobEnd;
                }
        ...
        $textwindow->bind('TextUnicode', '<Delete>' => sub{
                        my @ranges = $textwindow->tagRanges('sel');
                        my $range_total = @ranges;
                        if ($range_total){
                                $textwindow->addGlobStart;
        ...
        $textwindow->bind('TextUnicode', '<Control-t>' => sub{case('tc');$top->break});
        $textwindow->bind('TextUnicode', '<Control-Z>' => sub{$textwindow->undo; $textwindow->tagRemove('highlight','1.0','end');});
        $textwindow->bind('TextUnicode', '<Control-z>' => sub{$textwindow->undo; $textwindow->tagRemove('highlight','1.0','end');});
        $textwindow->bind('TextUnicode', '<Control-Y>' => sub{$textwindow->redo});
        $textwindow->bind('TextUnicode', '<Control-y>' => sub{$textwindow->redo});
        $textwindow->bind('TextUnicode', '<Control-f>' => \&searchpopup);
        $textwindow->bind('TextUnicode', '<Control-F>' => \&searchpopup);
        $textwindow->bind('TextUnicode', '<Control-p>' => \&gotopage);
        $textwindow->bind('TextUnicode', '<Control-P>' => \&gotopage);
        $textwindow->bind('TextUnicode', '<Control-w>' => sub{$textwindow->addGlobStart;floodfill();$textwindow->addGlobEnd;});
        $textwindow->bind('TextUnicode', '<Control-W>' => sub{$textwindow->addGlobStart;floodfill();$textwindow->addGlobEnd;});
        $textwindow->bind('TextUnicode', '<Control-Shift-exclam>' => sub{setbookmark('1')});
        $textwindow->bind('TextUnicode', '<Control-Shift-at>' => sub{setbookmark('2')});
        $textwindow->bind('TextUnicode', '<Control-Shift-numbersign>' => sub{setbookmark('3')});
        $textwindow->bind('TextUnicode', '<Control-Shift-dollar>' => sub{setbookmark('4')});
        $textwindow->bind('TextUnicode', '<Control-Shift-percent>' => sub{setbookmark('5')});
        $textwindow->bind('TextUnicode', '<Control-KeyPress-1>' => sub{gotobookmark('1')});
        $textwindow->bind('TextUnicode', '<Control-KeyPress-2>' => sub{gotobookmark('2')});
        $textwindow->bind('TextUnicode', '<Control-KeyPress-3>' => sub{gotobookmark('3')});
        $textwindow->bind('TextUnicode', '<Control-KeyPress-4>' => sub{gotobookmark('4')});
        $textwindow->bind('TextUnicode', '<Control-KeyPress-5>' => sub{gotobookmark('5')});
        $textwindow->bind('TextUnicode', '<Alt-Left>' => sub{$textwindow->addGlobStart; indent('out'); $textwindow->addGlobEnd;});
        $textwindow->bind('TextUnicode', '<Alt-Right>' => sub{$textwindow->addGlobStart; indent('in'); $textwindow->addGlobEnd;});
        $textwindow->bind('TextUnicode', '<Alt-Up>' => sub{$textwindow->addGlobStart; indent('up'); $textwindow->addGlobEnd;});
        $textwindow->bind('TextUnicode', '<Alt-Down>' => sub{$textwindow->addGlobStart; indent('dn'); $textwindow->addGlobEnd;});
        $textwindow->bind('TextUnicode', '<F7>' => \&spellchecker);
        $textwindow->bind('TextUnicode', '<Control-Alt-d>' => sub{
                        if ($DEBUG){
                                $DEBUG = 0;
                                rebuildmenu();
                        }else{
        ...
        $textwindow->bind('TextUnicode', '<Control-Alt-s>' => sub{
                        unless (-e 'scratchpad.txt'){
                                open my $fh, '>', 'scratchpad.txt' or warn "Could not create file $!";
                        }
        ...
        $textwindow->eventAdd('<<FindNext>>' => '<Control-Key-G>','<Control-Key-g>');
        $textwindow->bind('<<ScrollDismiss>>', \&scrolldismiss);
        $textwindow->bind('TextUnicode','<ButtonRelease-2>', sub {popscroll() unless $Tk::mouseMoved});
        $textwindow->bind('<<FindNext>>', sub {
                        if ($lglobal{search}){
                                my $searchterm = $lglobal{searchentry}->get('1.0','1.end');
        ...
        if (OS_Win){
                $textwindow->bind('TextUnicode', '<3>' => sub{scrolldismiss(); $menu->Popup( -popover => 'cursor' )});
        }else{
                $textwindow->bind('TextUnicode', '<3>' => sub{scrolldismiss()}); # Try to trap odd right click error under OSX and Linux
        }
        $textwindow->bind('TextUnicode', '<Control-Alt-h>' => \&hilitepopup);
        $textwindow->bind('TextUnicode', '<FocusIn>' => sub { $lglobal{hasfocus} = $textwindow });
...
sub buildstatusbar{
        $lglobal{current_line_label} = $counter_frame->Label(
                -text => 'Ln: 1 / 1  -  Col: 0',
                -width => 26, -relief => 'ridge',
                -background => 'gray',
        )->grid(-row => 1, -column => 0, -sticky => 'nw' );
        $lglobal{current_line_label}->bind('<1>', sub{
                        $lglobal{current_line_label}->configure(-relief => 'sunken');
                        gotoline();
                        update_indicators();
                }
        );
        $lglobal{current_line_label}->bind('<3>', sub{
                if ($vislnnm){$vislnnm = 0}else{$vislnnm = 1};
        ...
        $lglobal{selectionlabel}->bind('<1>', sub{
        ...
        $lglobal{selectionlabel}->bind('<3>', sub{
                        if ($textwindow->markExists('selstart')){
                                $textwindow->tagAdd('sel','selstart','selend');
                        }
                }
        );
        $lglobal{selectionlabel}->bind('<Shift-3>', sub{
                        $textwindow->tagRemove('sel','1.0','end');
                        if ($textwindow->markExists('selstart')){
        ...
        $lglobal{highlighlabel} = $counter_frame->Label(
                -text => 'H',
                -width => 2, -relief => 'ridge',
                -background => 'gray',
        )->grid( -row => 1, -column => 1);

        $lglobal{highlighlabel}->bind('<1>', sub{
                if($lglobal{scanno_hl}){
                        $lglobal{scanno_hl} = 0;
        ...
        $lglobal{highlighlabel}->bind('<Enter>', sub{
                        $lglobal{highlighttempcolor} = $lglobal{highlighlabel}->cget(-background);
                        $lglobal{highlighlabel}->configure(-background => $activecolor);
                        $lglobal{highlighlabel}->configure(-relief => 'raised')
                }
        );
        $lglobal{highlighlabel}->bind('<Leave>', sub{
                        $lglobal{highlighlabel}->configure(-background => $lglobal{highlighttempcolor});
                        $lglobal{highlighlabel}->configure(-relief => 'ridge')
                }
        );
        $lglobal{highlighlabel}->bind('<ButtonRelease-1>', sub{$lglobal{highlighlabel}->configure(-relief => 'raised')});
        ...
        $lglobal{insert_overstrike_mode_label}->bind('<1>', sub{
        ...
        $lglobal{ordinallabel}->bind('<1>', sub{
                        $lglobal{ordinallabel}->configure(-relief => 'sunken');
                        $lglobal{longordlabel} = $lglobal{longordlabel} ? 0 : 1;
                        update_indicators();
                }
        );
        butbind($_) for ($lglobal{insert_overstrike_mode_label}, $lglobal{current_line_label}, $lglobal{selectionlabel}, $lglobal{ordinallabel});
        $lglobal{statushelp} = $top->Balloon(-initwait => 1000);
        $lglobal{statushelp}->attach($lglobal{current_line_label}, -balloonmsg => "Line number out of total lines\nand column number of cursor.");
        $lglobal{statushelp}->attach($lglobal{insert_overstrike_mode_label}, -balloonmsg => 'Typeover Mode. (Insert/Overstrike)');
        $lglobal{statushelp}->attach($lglobal{ordinallabel}, -balloonmsg => "Decimal & Hexadecimal ordinal of the\ncharacter to the right of the cursor.");
        $lglobal{statushelp}->attach($lglobal{highlighlabel}, -balloonmsg => "Highlight words from list. Right click to select list");
        $lglobal{statushelp}->attach($lglobal{selectionlabel}, -balloonmsg => "Start and end points of selection -- Or, total lines.columns of selection");
}
...
sub greekpopup{
        ...
        if (defined($lglobal{grpop})){
                ...
                %attributes = (
                        ...
                        'cxi' => ['X', 'Xi', '&#926;', "\x{039E}"],
                        'lxi' => ['x', 'xi', '&#958;', "\x{03BE}"],
                        'nx' => ['nx', 'gamma xi', '&#947;&#958', "\x{03B3}\x{03BE}"],
                ...
                $tframe2->Button(
                        ...
                        -command => sub{
                                                        ...
                                                        if ($lglobal{grtext}->get('end -1c','end') =~ /^$/){$lglobal{grtext}->delete('end -1c','end')}
                                                },
                        -text => 'ASCII->Greek',
                ...
                $tframe2->Button(
                        ...
                        -command => sub{
                                                        ...
                                                        my $selection = $lglobal{grtext}->get($start,$end);
                                                        $lglobal{grtext}->delete($start,$end);
                                                        $lglobal{grtext}->insert($start,betagreek('unicode',$selection));
                                                        if ($lglobal{grtext}->get('end -1c','end') =~ /^$/){$lglobal{grtext}->delete('end -1c','end')}
                                                },
                ...
                $tframe2->Button(
                        ...
                        -command => sub{
                                                        ...
                                                        my $selection = $lglobal{grtext}->get($start,$end);
                                                        $lglobal{grtext}->delete($start,$end);
                                                        $lglobal{grtext}->insert($start,betagreek('beta',$selection));
                                                        if ($lglobal{grtext}->get('end -1c','end') =~ /^$/){$lglobal{grtext}->delete('end -1c','end')}
                                                },
                        -text => 'Unicode->Beta code',
                )->grid(-row => 1, -column => 4, -padx => 2);
                }
                my $frame = $lglobal{grpop}->Frame(-background => 'white')->pack(-expand => 'no' , -fill => 'none', -anchor => 'n');
                my $index = 0;
                for my $column(@greek){
                        my $row = 1;
                        $index++;
                ...
                my $bframe = $lglobal{grpop}->Frame->pack(-expand => 'yes' , -fill => 'both', -anchor => 'n');
                $lglobal{grtext} = $bframe->Scrolled(
                        ...
                        -setgrid => 'true',
                        -scrollbars => 'se',
                )-> pack(-expand => 'yes' , -fill => 'both',-anchor => 'nw',-pady => 5);        # autosizing
                $lglobal{grtext}->bind('<FocusIn>', sub { $lglobal{hasfocus} = $lglobal{grtext} });
                drag($lglobal{grtext});
                if ($Tk::version ge 8.4){
                        my $bframe2 = $lglobal{grpop}->Frame(-relief => 'ridge')->pack(-expand => 'n', -anchor => 's');
                        $bframe2->Label(
                        ...
                        $lglobal{buildentry}->bind('<FocusIn>', sub { $lglobal{hasfocus} = $lglobal{buildentry} });
                        $lglobal{buildentry}->bind($lglobal{buildentry}, '<Return>', sub{
                        ...
                        $lglobal{buildentry}->bind($lglobal{buildentry}, '<asciicircum>', sub{
                        ...
                        $lglobal{buildentry}->bind($lglobal{buildentry}, '<<alias>>' , sub{
                                                                        my $string = $lglobal{buildentry}->get;
                                                                        if ($string =~ /(^h$|^H$|^w$|^W$)/){
                                                                                $string =~ tr/WwHh/ÔôÊê/;
                                                                                $lglobal{buildentry}->delete('0','end');
                        ...
                        $lglobal{buildentry}->bind($lglobal{buildentry}, '<BackSpace>', sub{
                ...
                $lglobal{grpop}->protocol('WM_DELETE_WINDOW' => sub{
                        ...
                        %attributes = ();
                        $lglobal{grpop}->destroy; undef $lglobal{grpop}}
                );
...
sub pageadjust{
        if (defined $lglobal{padjpop}){
                ...
                $lglobal{padjpop}->protocol('WM_DELETE_WINDOW' => sub{
                ...
                my $frame0 = $lglobal{padjpop}->Frame->pack(-side => 'top', -anchor => 'n', -pady => 4);
                unless (@pages){
                        $frame0->Label(
                ...
                my $frame1 = $lglobal{padjpop}->Scrolled('Pane',
                        -scrollbars => 'se',
                        -background => 'white',
                )->pack(-expand => 1, -fill => 'both', -side => 'top', -anchor => 'n');
                drag($frame1);
                $top->update;
                my $updatetemp;
                $top->Busy(-recurse => 1);
...
sub pnumadjust{ #pop up a window to edit/resequence page markers
        ...
        if ($lglobal{pnumpop}){
                ...
                $lglobal{pnumpop}->geometry($lglobal{pnpopgoem}) if $lglobal{pnpopgoem};
                ...
                $lglobal{pnumpop}->bind($lglobal{pnumpop}, '<Delete>' => \&pageremove);
                $lglobal{pnumpop}->protocol('WM_DELETE_WINDOW' => sub{
                ...
                if (OS_Win){
                        $lglobal{pagerenumoffset}->bind($lglobal{pagerenumoffset}, '<MouseWheel>' => [ sub {($_[1]>0) ? $lglobal{pagerenumoffset}->invoke('buttonup') : $lglobal{pagerenumoffset}->invoke('buttondown')}, Ev('D') ]);
                }
        }
}
...
sub pgrenum{ # Re sequence page markers
        ...
        if ($offset<0){
                ...
                while($num = $textwindow->markPrevious($num)){
                        if ($num =~ /Pg\d+/){
                                $mark = $num;
                                $mark =~ s/Pg(\d+)/$1/;
                                if (($mark - $start) le $offset){$offset = ($mark - $start+1)};
                                last;
                        }
                }
                while (!($textwindow->markExists($marks[$#marks]))){pop @marks};
                $end = $marks[$#marks];
        ...
        for (@marks){$pagenumbers{$_}{offset} = $textwindow->index($_) if $_ =~ /Pg\d+/;}
        $lglobal{seepagenums} = 0;
        viewpagenums();
}
...
sub pmoveup{ # move the page marker up a line
        ...
        if ($num eq '1.0'){
                ...
                return if $textwindow->compare($index,'<',($textwindow->index($num.'+'.length($pagenum).'c')));
        }
...
sub pmoveright{ # move the page marker left a character
        ...
        my $pagenum = " $mark ";
        my $index = $textwindow->index("$mark+1c");
        if ($textwindow->compare($index,'>=',$textwindow->index($mark).'lineend -'.length($pagenum).'c')){$index = $textwindow->index($textwindow->index($mark).' +1l linestart')};
        if ($textwindow->compare($num,'==','end')){
                return if $textwindow->compare($index,'>=','end');
        }else{
                return if $textwindow->compare($index.'+'.length($pagenum).'c','>=',$num);
        }
        $textwindow->ntdelete($mark,$mark.' +'.length($pagenum).'c');
        $textwindow->markSet($mark,$index);
        $textwindow->markGravity($mark,'left');
...
;
                ...
                for (qw/activecolor auto_page_marks autobackup autosave autosaveinterval blocklmargin blockrmargin
                        ...
                        print $save_handle "\$$_", ' ' x (20 - length $_), "= '", eval '$'.$_, "';\n";
                };
                print $save_handle "\n";

                for (qw/globallastpath globalspellpath globalspelldictopt globalviewerpath globalbrowserstart
                                gutpath jeebiespath scannospath tidycommand/) {
                        print $save_handle "\$$_", ' ' x (20 - length $_), "= '", escape_problems(os_normal(eval '$'.$_)), "';\n";
                }
...
sub uchar{
        if (defined $lglobal{ucharpop}){
                ...
                $lglobal{ucharpop}->protocol('WM_DELETE_WINDOW' => sub{$lglobal{ucharpop}->destroy; undef $lglobal{ucharpop};});
                $lglobal{ucharpop}->Icon(-image => $icon);
                my $cframe = $lglobal{ucharpop}->Frame->pack;
                my $frame0 = $lglobal{ucharpop}->Frame->pack(-side => 'top', -anchor => 'n', -pady => 4);
                my $sizelabel;
                my (@textchars, @textlabels);
                my $pane = $lglobal{ucharpop}->Scrolled('Pane',
                        -background => 'white',
                        -scrollbars => 'se',
                        -sticky => 'wne',
                                                        ...
                                                        while (@lines){
                                                                ...
                                                                for my $char(@chars){
                                                                        ...
                                                                        if (@chars == $count){
                                                                                my $block = '';
                                                                                for (keys %blocks){
                                                                                        if (hex ($blocks{$_}[0]) <= hex ($ord) && hex($blocks{$_}[1]) >= hex ($ord)){
                                                                                                $block = $_;
                                                                                                last;
                                                                                        }
                                                                                }
                                                                                ...
                                                                                utflabel_bind($textlabels[$row], $block, $blocks{$block}[0], $blocks{$block}[1]);
                                                                                $row++;
                                                                        }
                                                                }
                                                        }
...
sub utflabel_bind{
        ...
        $widget->bind('<Enter>', sub{$widget->configure(-background => $activecolor);});
        $widget->bind('<Leave>', sub{$widget->configure(-background => 'white');});
        $widget->bind('<ButtonPress-1>', sub{
...
sub drag{
        ...
        my $corner_label = $corner->Label(
                -image => $lglobal{drag_img}
        )->pack(-side => 'bottom', -anchor => 'se');
        $corner_label->bind('<Enter>', sub {
                if (OS_Win){
        ...
        $corner_label->bind('<1>', sub {($lglobal{x}, $lglobal{y}) = ($scrolledwidget->toplevel->pointerx, $scrolledwidget->toplevel->pointery)});
        $corner_label->bind('<B1-Motion>', sub {
                my $x = $scrolledwidget->toplevel->width  - $lglobal{x} + $scrolledwidget->toplevel->pointerx;
                my $y = $scrolledwidget->toplevel->height - $lglobal{y} + $scrolledwidget->toplevel->pointery;
                ($lglobal{x}, $lglobal{y}) = ($scrolledwidget->toplevel->pointerx, $scrolledwidget->toplevel->pointery);
        ...
        if(open my $fh, '-|', "$jeebiespath $jeebiesoptions $title"){
                while (my $line = <$fh>){
                        ...
                        if ($line){
                                ...
                                $mark++ if $linenum;
                                $textwindow->markSet("j$mark", "$linenum.$colnum") if $linenum;
                                $jeeb{$line} = "j$mark";
...
sub jeebiespop_up{
        ...
        if ($lglobal{jeepop}){
                ...
                $lglobal{jelistbox} = $pframe->Scrolled('Listbox',
                        -scrollbars => 'se',
                        -background => 'white',
                        -font => $lglobal{font},
                        -selectmode => 'single',
                ...
                $lglobal{jeepop}->protocol('WM_DELETE_WINDOW' => sub{ $lglobal{jeepop}->destroy; undef $lglobal{jeepop}});
                ...
                $lglobal{jeepop}->bind('<Configure>' => sub {
                        $lglobal{jeepop}->XEvent;
                        $geometry2 = $lglobal{jeepop}->geometry;
                        $lglobal{geometryupdate} = 1;
                        }
                );
                $lglobal{jelistbox}->eventAdd('<<jremove>>' => '<ButtonRelease-2>','<ButtonRelease-3>');
                $lglobal{jelistbox}->bind('<<jremove>>', sub {
                        $lglobal{jelistbox}->activate($lglobal{jelistbox}->index
                        ('@'.($lglobal{jelistbox}->pointerx - $lglobal{jelistbox}->rootx).','.($lglobal{jelistbox}->pointery-$lglobal{jelistbox}->rooty)));
...
sub blocks_check{
        ...
        my $oops = $top->DialogBox(
                -buttons => [qw[Yes No]],-title => 'Critical files missing.',
                -popover => $top,
                -command => sub {
                                if ($_[0] eq 'Yes'){
        ...
        $oops->add('Label',
                -text => "Your Perl installation is missing some files\nthat are critical for some Unicode operations.\n".
...
sub natural_sort_alpha {        # Ultra fast natural sort - wants an array
        my $i;
        s/(\d+(,\d+)*)/pack 'aNa*', 0, length $1, $1/eg, $_ .= ' ' . $i++ for (my @x = map {lc deaccent $_} @_);
        @_[map {(split)[-1]} sort @x];
}

sub natural_sort_length {       # Fast length sort with secondary natural sort - wants an array
        $_->[2] =~ s/(\d+(,\d+)*)/pack 'aNa*', 0, length $1, $1/eg for (my @x = map { [length noast($_), $_, lc deaccent $_ ]} @_);
        map { $_->[1] } sort { $b->[0] <=> $a->[0] or $a->[2] cmp $b->[2] }@x;
}

sub natural_sort_freq {         # Fast freqency sort with secondary natural sort - wants a hash reference
